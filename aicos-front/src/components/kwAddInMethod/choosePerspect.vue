<template>
  <el-row class="perspect-manage-container">
    <div class="fag-manage-tree" :style="{ height: allWindowTopBodyHeight + 'px' }">
      <el-scrollbar style="border-right: 10px solid #f8f9fa">
        <div class="profile-config-template-scroll">
          <ul id="profileConfigList" class="profile-config-template-list">
            <li class="profile-config-item" v-for="list in viewTypeList" :key="list.id"
              :class="[selectedCateId == list.id ? 'item-this' : '']" :title="list.name"
          ...
          <div class="kw-header">
            <div id="kw-form-container" style="margin: 0px; padding: 0">
              <div class="kw-form-form">
                <el-input size="small" :placeholder="$t('answerViewManage.viewName')" v-model="viewName" clearable
                  class="kw-form-type">
                </el-input>
              </div>
              <div class="kw-form-btn">
                <el-button size="small" type="primary" class="kw-search-btn" icon="el-icon-search"
        ...
        <div class="chooseed-cate-item">
          ...
          <ul>
            <li v-for="(perspectItem, i) in curSelectedPerspects" :key="i">
              <span class="perspect-tag-text" :title="perspectItem.cateName + '：' + perspectItem.name">{{
