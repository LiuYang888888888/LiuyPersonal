<template>
  <basic-container class="kwpolicy-content">
    ...
    <avue-crud :option="userQuestionTableOption" :table-loading="loading" :data="userQuestionTableData"
      :page.sync="userQuestionTablePage" ref="kwpolicyTable" :permission="permissionList"
...
import AddQuestioning from "./addQuestioning.vue";
export default {
  //获取父页面传的数据
  props: {
    questionType: {
  ...
  data () {
    return {
      ...
      userQuestionTableOption: {
        ...
        column: [
          {
            label: this.$t("kwRobotManage.filterQuestion") + this.$t('kwRobotManage.contain'),
            prop: "content",
            span: 12,
            minWidth: 280,
            showWordLimit: true,
