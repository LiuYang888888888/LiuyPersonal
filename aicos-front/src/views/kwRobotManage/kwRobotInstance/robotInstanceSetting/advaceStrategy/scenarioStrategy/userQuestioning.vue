<template>
  <div>
          ...
          <div class="kwpolicy-search-input-row">
            <div class="kwpolicy-search-value">
              <div class="kwpolicy-search-input">
                <el-input
                  :placeholder="$t('kwRobotManage.searchContentTips')"
                  size="small"
    ...
    <avue-crud
      :option="userQuestionTableOption"
      :table-loading="loading"
      :data="userQuestionTableData"
      ref="kwpolicyTable"
    ...
    >
      <template slot-scope="scope" slot="menu">
        <el-button
          type="text"
          size="small"
          @click.stop="openEditQuestion(scope.row, scope.index)"
        >
    ...
    <HuilanDialog
      append-to-body
      :visible.sync="isAddQuestioningDialog"
      width="600px"
      trigger="click"
    >
      <AddQuestioning
        v-if="isAddQuestioningDialog"
      ...
      <template v-slot:footer>
        ...
        <el-button
          size="small"
          type="primary"
          @click="saveAddUpdataQuestioning"
    ...
    <HuilanDialog
      append-to-body
      :visible.sync="isEditQuestioningDialog"
      width="600px"
      trigger="click"
    >
      <AddQuestioning
        v-if="isEditQuestioningDialog"
        :questioningId="questioningId"
        :questioningType="'edit'"
        :questionType="questionType"
        :sceneType="sceneType"
        :strategyId="strategyId"
        :questioningObj="questioningObj"
        @addOrEditEmitEvent="addOrEditEmitEvent"
        ref="addEditQuestioning"
      />
      <template v-slot:footer>
...
export default {
  ...
  data() {
    return {
      ...
      copyuserQuestionTableData: [],
      userQuestionTableData: [],
      userQuestionTableOption: {
        ...
        column: [
          {
            label:
              this.$t("kwRobotManage.userQuestions") +
              this.$t("kwRobotManage.contain"),
            prop: "content",
            span: 12,
            minWidth: 280,
            showWordLimit: true,
  ...
  mounted() {
    ...
    // this.onLoad();
    this.$nextTick(() => {
      this.userQuestionTableData = deepClone(this.userData);
      this.copyuserQuestionTableData = deepClone(this.userData);
    });
  },
  components: {
    AddQuestioning
  ...
  methods: {
    ...
    delQuestion(row, index, type) {
      ...
      // };
      this.$confirm(
        `${this.$t("kwRobotManage.deletionTips")}${isOnlyStr} ${this.$t(
