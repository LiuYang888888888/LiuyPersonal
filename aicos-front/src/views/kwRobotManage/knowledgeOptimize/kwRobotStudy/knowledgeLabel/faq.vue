<template>
  <el-row class="perspect-manage-container-faq">
    <div class="fag-manage-tree" :style="{ height: allWindowTopBodyHeight + 'px' }" style="overflow-y: auto">
      <LeftTree ref="tree" @getNode="getNode" @nodeClick="nodeClick" :classType="classType" :botId="botId" :isMine="false"
        :isSetting="false" :name="$t('faq.knowledgeClass')" />
    </div>
    <basic-container :style="{ height: allWindowTopBodyHeight + 'px' }" class="fag-manage-content">
      <avue-crud :option="faqTableOption" :data="faqTableData" ref="crudFaqTable" :page.sync="faqTablePage"
          ...
          <div class="kw-header">
            <div id="kw-form-container" style="margin: 0px; padding: 0">
              <div class="kw-form-form">
                <el-input size="small" :placeholder="$t('aicosCommon.pleaseInput') + $t('faq.question')
                  " v-model="queryObj.content" clearable class="kw-form-type">
                </el-input>
              </div>
              <div class="kw-form-btn">
                <el-button size="small" type="primary" icon="el-icon-search" class="kw-search-btn"
        ...
        <template slot-scope="scope" slot="status">
          <span v-if="scope.row.status == 0">{{ $t('faq.draft') }} </span>
          <span v-else-if="scope.row.status == 1">{{ $t('faq.release') }} </span>
        </template>
        <template slot-scope="scope" slot="enable">
          <span v-if="scope.row.enable == 0">{{ $t('faq.deactivate') }} </span>
          <span v-else-if="scope.row.enable == 1"> {{ $t('faq.enable') }} </span>
        </template>
      </avue-crud>
    </basic-container>
  </el-row>
...
import { loadListUrl } from "@/api/kwKnowledgeCenter/kwKnowledgeMangement/kwFaqManagement/index.js";
...
.perspect-manage-container-faq {
  ...
  .fag-manage-content .el-card__body {
    padding: 0;
  }
