  ...
  <div style="padding: 20px;">
    ...
    >
      ...
      <el-form-item size="large" class="btnContainer" v-if="!this.editNode.id">
        ...
        >
        <!-- <el-button @click="goBack" class="kw-normal-button">{{
    ...
    >
      <el-tabs v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane
          :label="
            `${$t('kwIntentionManage.regular')}${`(${this.regularArr.length})`}`
          "
          name="second"
        >
          <Regular
...
export default {
  ...
  watch: {},
  async mounted() {
    this.formData.intentionName = this.rowData.question;
    // if (this.sonCur.id) {
    //   this.flag = true;
    // } else {
    //   this.flag = false;
  ...
  methods: {
    ...
    async submitForm() {
      this.$refs.elForm.validate(async valid => {
        if (!valid) {
          ...
          if (!this.validatenull(this.formData.dialogueProcessNodeIdIMAssist)) {
            let dialogueType = this.dialogueOptions.find(
              ...
              dialogueProcessNodeName = this.nodeOptionsIMAssist.find(
                item => item.id === this.formData.dialogueProcessNodeIdIMAssist
              ).nodeName;
            newInfo.dialogueProcessList.push({
              dialogueProcessId: this.formData.dialogueProcessIdIM,
              dialogueProcessNodeId: this.formData.dialogueProcessNodeIdIM,
          ...
          if (!this.isHaveSameNameFlag) {
            this.disable = true;
            const result = await saveOrUpdate(newInfo);
            if (result.data.code == 200) {
              this.$message({
                type: "success",
    ...
    saveSimilarExpand() {
      this.recommendList.faq = this.$refs.kwSimilarRef.$refs.similarContainerRef.saveSimilarQuesion();
      this.recommendList.tab = this.$refs.kwSimilarRef.$refs.similarTabcontainerRef.saveSimilarQuesion();
