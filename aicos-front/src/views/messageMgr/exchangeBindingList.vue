...
import { bindings, unbindings } from '@/api/messageMgr/queueMgr'
import { getSourceBindingsList } from '@/api/messageMgr/exchangeMgr'

export default {
  name: 'bindingQueueList',
  props: {
    item: {
      type: Object,
      default() {
  ...
  methods: {
    ...
    onLoad() {
      ...
      getSourceBindingsList(this.item.name).then(res => {
      ...
      this.$confirm("确定将选择数据解绑?", {
        ...
        .then(() => {
          const data = {
            ...
            destinationType: row.destination_type === 'exchange' ? 'e' : 'q',
