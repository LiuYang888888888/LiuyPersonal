<template>
  <van-form @submit="onSubmit">
    <div v-for="(item, index) in list" :key="index">
      ...
      <van-popup v-model="item.show" position="bottom" v-if="item.type == 'danwei'">
        <div class="van-picker__toolbar">
          <button type="button" class="van-picker__cancel" @click="showListcancle(item)">
            取消
          </button>
...
export default {
  ...
  methods: {
        ...
        if(item.type == 'danwei'){
          ...
          danwei(params).then(res=>{
            if (res.data && res.data.data) {
              ...
              console.error('danwei response is missing expected data:', res.data);
            }
          }).catch(error => {
            console.error('Error fetching danwei data:', error);
          });
        }
    ...
    showListvalue(value) {
      ...
      } else if (this.list[this.index].type == 'danwei') {
        if (this.checkboxValue.length > 0) {
          this.options[prop] = this.checkboxValue.join(',');
        } else {
          this.options[prop] = '';
