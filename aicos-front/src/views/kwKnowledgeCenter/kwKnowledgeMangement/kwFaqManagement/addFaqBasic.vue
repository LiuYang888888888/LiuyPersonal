<template>
  <div>
    ...
    >
      <template v-slot:caName>
        {{ curTreeNode.data && curTreeNode.data.dname }}
      </template>
      <template slot="question">
        <el-input
          type="input"
          :placeholder="
            $t('aicosCommon.pleaseInput') + $t('faq.standardIssues')
          "
      ...
      <template v-slot:cateId>
        {{ curTreeNode.data && curTreeNode.data.id }}
      </template>
      <template v-slot:extQuestions>
        <el-button
          type="text"
          @click.stop="similarDialogVisible = true"
          v-if="faqType !== 'view' && faqType !== 'version'"
          >{{ $t("faq.addQuestions") }}</el-button
        >
        <span style="padding-left: 10px"
          >{{ $t("faq.added") }}：{{ similarQuestionLength }}</span
        >
        <el-button
      ...
      <template v-slot:formAnswers>
        <div>
          <div
            class="answer-perspective-item"
            v-for="(
              formAnswerItem, answerIndex
            ) in faqBasicFormData.formAnswers"
            :key="answerIndex"
          >
              ...
              >
                <div class="perspect-list-wrap">
                  <ul class="perspect-list-ul">
                    ...
                    >
                      <template v-if="onlyPerspectvie.cateName">
                        <span
                      ...
                      <template v-else>
                        <span
                          class="perspect-tag-text"
                          :title="onlyPerspectvie.name"
                          >{{ onlyPerspectvie.name }}</span
                        >
                      </template>
                      <i
                ...
                <el-button
                  ...
                  >{{ $t("faq.chooseAPerspective") }}</el-button
                >
              </div>
              <div class="perspect-list-container" v-else>
                <p style="margin: 0">{{ $t("faq.universalPerspective") }}</p>
              </div>
            ...
            <el-button
              v-if="
                faqBasicFormData.formAnswers.length > 1 &&
                answerIndex != 0 &&
                faqType !== 'view' &&
                faqType !== 'version'
              "
      ...
      <template v-slot:addAnswerSpectiveBtn>
        <el-button icon="el-icon-plus" type="text" @click="addAnswer">{{
          $t("faq.addAnswer")
        }}</el-button>
      </template>
      <!-- <template v-slot:intentGroupLabel>
        <span>{{ $t("faq.relatedIntention") }}：</span>
      </template> -->
      <template v-slot:intentGroup>
        <div>
          <el-autocomplete
            ...
            :placeholder="$t('faq.intentionTips')"
          ></el-autocomplete>
          <el-tooltip
            class="item"
            effect="light"
    ...
    >
      <el-tabs v-model="angleActiveName" @tab-click="handleClick">
        <el-tab-pane :label="$t('faq.channelPerspective')" name="one">
          <ChooseChannel
        ...
        <el-tab-pane :label="$t('faq.customPerspective')" name="two">
          <ChoosePerspect
            v-if="choosePerspectVisible"
        ...
        <el-tab-pane :label="$t('faq.customerPerspective')" name="three">
          <ChooseClient
            v-if="choosePerspectVisible"
            ref="chooseClientRef"
    ...
    <!-- 大模型自动扩写 -->
    <HuilanDialog
      :title="$t('faq.expandDialogTitle')"
      :close-on-click-modal="isCloseOnClickModal"
      append-to-body
      :visible.sync="modelExpandVisible"
      width="55%"
      custom-class="choose-perspective-dialog"
    >
      <div class="expandModule">
        <div class="expandTip">
          <div class="tip">
            {{ $t("faq.expandDialogTip") }}
          </div>
          <el-button icon="el-icon-refresh" type="text" @click.stop="kWFaqExtendRefresh">{{
            $t("faq.expandRefresh")
          }}</el-button>
        </div>
        <el-checkbox-group v-model="checkedModule" v-loading='kWFaqExtendLoading' v-if="checkOption.length > 0">
          <el-checkbox
            v-for="(item, i) in checkOption"
            :label="item"
            :key="item"
          >
            ...
            <template v-else> {{ i + 1 }}、{{ item }} </template>
          </el-checkbox>
        </el-checkbox-group>
        <div class="expandModule-empty" v-else>
          <avue-empty v-loading='kWFaqExtendLoading'></avue-empty>
        </div>
      </div>
      <template v-slot:footer>
        <el-button size="small" @click="modelExpandVisible = false">{{
          $t("aicosCommon.cancelBtn")
        }}</el-button>
        <el-button size="small" type="primary" @click="saveAddQuestion">{{
          $t("faq.addExpandBtn")
        }}</el-button>
      </template>
    </HuilanDialog>
...
import { kWFaqExtendApi } from '@/api/kwKnowledgeCenter/kwKnowledgeMangement/kwFaqManagement/index';
...
export default {
    ...
    return {
      ...
      faqBasicFormOption: {
        column: [
            ...
            rules: [
              {
                ...
                message:
                  ...
                  this.$t("faq.standardIssues"),
              },
              // ...getRules(["charNumHanCommaStop"]),
            ],
          },
          {
            type: "input",
          ...
          {
            type: "input",
            label: this.$t("faq.relatedIntention"),
            display: true,
            span: 24,
            ...
            prop: "intentGroup",
            rules: [
              { required: true, message: this.$t("faq.intentionTips") },
              // ...getRules(["charNumHan"]),
            ],
          },
          {
            label: this.$t("faq.answerPerspective"),
            // label:'',
            // labelWidth: "0",
            display: true,
            span: 22,
  ...
  mounted() {
    // console.log("faqType", this.faqType);
    ...
    getDictionary({ code: "faq_similarityQuestion_smart" }).then((res) => {
      if (res.data.code == 200) {
        this.isOpen = JSON.parse(res.data.data[0].value.toLowerCase());
      }
    });
  },
  methods: {
    expandClick() {
      if(!this.faqBasicFormData.question){
        ...
        this.$message({
          type: "warning",
          message: this.$t("faq.mustaStandardIssues"),
        });
        return false;
      }
      this.modelExpandVisible = true;
      this.checkOption = [];
      this.checkedModule = [];
      this.kWFaqExtend();
    },
    // 保存扩展的相似问
    saveAddQuestion() {
      let checkedModule = this.deepClone(this.checkedModule),
        extquestion = [],
        arr = [];
      this.deepClone(this.frequentlyQuestions).map((item) => {
        extquestion.push(item.intentQuestion);
      });
      if (extquestion.length > 0) {
        checkedModule.map((item) => {
          if (!extquestion.includes(item)) {
            arr.push({ intentQuestion: item });
          }
        });
        this.frequentlyQuestions = [...this.frequentlyQuestions, ...arr];
      } else {
        checkedModule.map((item) => {
          this.frequentlyQuestions.push({ intentQuestion: item });
        });
      }
      this.similarQuestionLength =
        this.regularArr.length + this.frequentlyQuestions.length;
      this.modelExpandVisible = false;
    },
    onBlurQuestion() {
      let obj = {
    ...
    setFaqBasicData(faqData) {
      ...
      var faqquestion = this.basicDataObj.faqquestion;
      var extquestion = this.basicDataObj.extquestion; // 相似问法
      var regquestion = this.basicDataObj.regquestion; // 正则
      var faqanswer = this.basicDataObj.faqanswer; // 答案
      // var inteList = this.basicDataObj.inteList; // 意图
      // var formAnswers = [];
    ...
    deletePersItem(onlyPerspectvie, answerIndex) {
      ...
      if (type === "channel") {
        ...
        that.faqBasicFormData.formAnswers[answerIndex];
      }
      // 删除自定义视角
      if (type === "selfDefined") {
    ...
    saveFaqBasic() {
        ...
        if (index === 0) {
          if (item.answers === "") {
            if (document.getElementsByClassName("el-message").length === 0) {
              this.$message({
                type: "warning",
                message: this.$t("faq.fillAnswerTips"),
              });
            }
            flag = true;
          }
        } else {
          if (this.validatenull(item.channel)) {
            flag = true;
            if (document.getElementsByClassName("el-message").length === 0) {
              this.$message.warning(this.$t("faq.channelTip"));
            }
          ...
          if (
            this.validatenull(item.answers) &&
            this.validatenull(item.perspectives)
          ) {
            flag = true;
            if (document.getElementsByClassName("el-message").length === 0) {
              this.$message({
                type: "warning",
                message: this.$t("faq.answerOrPerspectiveTips"),
              });
            }
          } else {
            if (this.validatenull(item.answers)) {
              flag = true;
              if (document.getElementsByClassName("el-message").length === 0) {
                this.$message({
                  type: "warning",
            ...
            if (this.validatenull(item.perspectives)) {
              flag = true;
              if (document.getElementsByClassName("el-message").length === 0) {
                this.$message({
                  type: "warning",
                  message: this.$t("faq.perspectiveTips"),
                });
              }
            }
          }
      ...
      if (listLength > newListLength) {
        this.$message.warning(this.$t("faq.repeatedTips"));
        flag = true;
      }
      ...
      basicData.faqquestion = {
        ...
        cateId: this.curTreeNode.data.id,
    ...
    kWFaqExtend(){
      ...
      kWFaqExtendApi(dataParams).then((res)=>{
        // console.log(res.data.data.data.results);
        let resData = res.data.data.data.results || [];
        this.kWFaqExtendLoading = false;
        this.checkOption = [];
        resData.map((item)=>{
          let sliceIndex = item.indexOf(".");
          if(sliceIndex > 0 ){
            this.checkOption.push( (item.slice(sliceIndex+1)+'').trim());
          }else{
            this.checkOption.push( (item+'').trim() );
          }
        });
      });
    },
