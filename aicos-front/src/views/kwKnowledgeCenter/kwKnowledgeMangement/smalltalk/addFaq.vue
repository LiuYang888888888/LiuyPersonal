<template>
  <div class="kw-tabs-base-container">
    <el-tabs
      v-model="activeName"
      :style="{
        height: allWindowTopBodyHeight - 72 + 'px',
        overflowY: 'auto',
      }"
    >
...
export default {
  ...
  methods: {
    ...
    // 基础设置faq保存
    saveFaqBasic(res) {
      let faqBasicForm = this.$refs.addFaqBasic.$refs.faqBasicFormRef;
    ...
    saveFaqMore(res) {
      let faqMoreForm = this.$refs.addFaqMore.$refs.faqMoreFormRef;
    ...
    submitFaq(isDeplay, startAudit = false) {
      ...
      for (let i in that.basicData.faqanswer) {
        if (that.basicData.faqanswer[i].content.length > 2000) {
