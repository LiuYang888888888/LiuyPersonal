<template>
  <HuilanBasic :left-width="250">
      ...
      >
        <basic-container>
          <LeftTree
            ...
            :name="$t('smalltalk.chattingClass')"
          />
        </basic-container>
      </div>
    </template>
          ...
          <div class="faq-search-header-wrap clearfix">
            <div class="faq-operation-btns">
              <el-button
                ...
                >{{ $t("smalltalk.addKnowledge") }}</el-button
              >
              <el-button
                size="small"
                class="kw-build-button kw-btn-purple"
                v-if="
                  curTreeNodeId !== '0' &&
                  curTreeNodeId !== 'mine' &&
                  curTreeNodeId !== '' &&
                  permissionList.dialogueBtn
                "
                @click="generateDialog"
              >
                {{ $t("smalltalk.generativeDialogue") }}：{{ switchText }}
              </el-button>
              <el-dropdown
                v-if="
                  permission[`${prefix}chat-startMore`] ||
                  permission[`${prefix}chat-stopMore`] ||
                  permission[`${prefix}chat-deleteMore`] ||
                  permission[`${prefix}chat-export`]
                "
              ...
              >
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    command="enable"
                    v-if="permission[`${prefix}chat-startMore`]"
                    >{{ $t("aicosCommon.batchStart") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="stop"
                    v-if="permission[`${prefix}chat-stopMore`]"
                    >{{ $t("aicosCommon.batchPause") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="del"
                    v-if="permission[`${prefix}chat-deleteMore`]"
                  ...
                  <el-dropdown-item
                    command="sync"
                    v-if="
                      permission[`${prefix}chat-sync`] &&
                      curTreeNodeId != '0' &&
                      curTreeNodeId != 'mine' &&
                      curTreeNodeId != ''
                    "
                    >{{ $t("smalltalk.syncKnowledge") }}</el-dropdown-item
                  >
              ...
              <div class="faq-search-input-row">
                ...
                <div class="faq-search-value">
                  <div class="faq-search-input">
                    <el-input
                      :placeholder="
                        $t('aicosCommon.pleaseInput') + $t('smalltalk.keywords')
                      "
                      size="small"
    ...
    <el-drawer
      :title="
        faqManageType == 'add'
          ? this.$t('aicosCommon.addBtn')
          : faqManageType == 'edit'
          ? this.$t('aicosCommon.editBtn')
          : this.$t('aicosCommon.viewBtn')
      "
    ...
    >
      <TreeSetting
        ...
        :classType="classType"
        v-if="settingDrawer"
        :treeRootName="$t('smalltalk.generalClass')"
      />
    ...
    <!--版本记录-->
    <HuilanDialog
      :title="$t('smalltalk.versionRecord')"
      append-to-body
      trigger="click"
    ...
    <!-- 生成式对话弹窗 -->
    <HuilanDialog
      :title="$t('smalltalk.generativeDialogue')"
      append-to-body
    ...
    >
      <div>
        <div>
          <span>{{ $t("smalltalk.generativeDialogue") }}：</span>
          <el-switch
            v-model="generateSwitch"
            active-color="#13ce66"
            active-value="1"
            inactive-value="0"
          >
        ...
        <div
          ...
          "
        >
          {{ $t("smalltalk.generativeDialogueTips") }}
        </div>
      </div>
      <template v-slot:footer>
        <el-button size="small" @click="generateDialogVisible = false">{{
          $t("aicosCommon.cancelBtn")
        }}</el-button>
        <el-button size="small" type="primary" @click="handlerSaveSwitch">{{
          $t("aicosCommon.saveBtn")
        }}</el-button>
      </template>
    </HuilanDialog>
...
export default {
  ...
  data() {
    return {
      ...
      drawerVisible: false,
      // 生成式对话
      switchText: this.$t("smalltalk.open"),
      ...
      searchStatusOptions: [
        {
          label: this.$t("smalltalk.release"),
          value: "1",
        },
        {
          label: this.$t("smalltalk.draft"),
          value: "0",
        },
      ],
      searchEnableOptions: [
        {
          label: this.$t("smalltalk.enable"),
          value: 1,
        },
        {
          label: this.$t("smalltalk.deactivate"),
          value: 0,
        },
      ],
      searchAvaliableStatusOptions: [
        ...
        {
          label: this.$t("smalltalk.inEffect"),
          value: "1",
        },
        {
          label: this.$t("smalltalk.expired"),
          value: "2",
        },
      ],
      loading: false,
      ...
      faqTableOption: {
        ...
        column: [
          ...
          // },
          {
            label: this.$t("smalltalk.knowledgeState"),
            prop: "status",
            minWidth: 100,
            type: "select",
            dicData: [
              ...
              {
                label: this.$t("smalltalk.published"),
                value: 1,
              },
            ],
            showWordLimit: true,
          ...
          {
            label: this.$t("smalltalk.startStopStatus"),
            prop: "enable",
            minWidth: 100,
            type: "select",
            dicData: [
  ...
  computed: {
    ...
    // 计算属性
    ...mapGetters(["botObj", "permission", "userInfo", "language"]),
    permissionList() {
      return {
        dialogueBtn: this.vaildData(
          this.permission[`${this.prefix}chat-switch`],
          false
        ),
        addBtn: this.vaildData(
          this.permission[`${this.prefix}chat-add`],
          false
        ),
        editBtn: this.vaildData(
          this.permission[`${this.prefix}chat-edit`],
          false
        ),
        viewBtn: this.vaildData(
          this.permission[`${this.prefix}chat-view`],
          false
        ),
        delBtn: this.vaildData(
          this.permission[`${this.prefix}chat-delete`],
          false
        ),
        publishBtn: this.vaildData(
          this.permission[`${this.prefix}chat-publish`],
          false
        ),
        stopBtn: this.vaildData(
          this.permission[`${this.prefix}chat-stop`],
          false
        ),
        startBtn: this.vaildData(
          this.permission[`${this.prefix}chat-start`],
          false
        ),
        versionsBtn: this.vaildData(
          this.permission[`${this.prefix}chat-colophon`],
          false
        ),
        topBtn: this.vaildData(
          this.permission[`${this.prefix}chat-top`],
          false
        ),
        cancelTopBtn: this.vaildData(
          this.permission[`${this.prefix}chat-canceltop`],
          false
        ),
        viewRecodeBtn: true,
        closeAuditBtn: true,
  ...
  methods: {
    async getAuditConfig() {
      ...
      this.auditSetting = await getAuditSetting(data);
    ...
    getRowActions(row) {
      ...
      if (row.auditStatus == "auditing" && this.isAudit) {
        map.closeAuditBtn = {
          name: this.$t("smalltalk.withdraw"),
          command: "closeAudit",
        };
        list.push("closeAuditBtn");
      }
      if (row.status == 0 && this.validatenull(row.auditStatus)) {
        map.publishBtn = {
          name: this.$t("smalltalk.release"),
          command: "publishFaq",
        };
      ...
      if (row.enable == 1) {
        map.stopBtn = {
          name: this.$t("smalltalk.deactivate"),
      ...
      if (row.questionOrder === 1) {
        map.cancelTopBtn = {
          name: this.$t("smalltalk.cancelTopping"),
          command: "updateTopStatus",
        };
      ...
      if (
        ...
        map.viewRecodeBtn = {
          name: this.$t("smalltalk.auditRecords"),
      ...
      if (
        ...
        map.versionsBtn = {
          name: this.$t("smalltalk.versionRecord"),
          command: "openVersionRecordDialog",
        };
    ...
    sureSearchDismension() {
      ...
      if (dialogDismensionData.length === 1) {
        this.$message({
          message: this.$t("smalltalk.secondaryTips"),
          type: "warning",
        });
        return false;
      }
    ...
    // 生成式对话这状态保存
    handlerSaveSwitch() {
      updateChart(this.curSelectedTreeNode.data.id, this.generateSwitch).then(
        (res) => {
          if (res.data.success) {
            if (this.generateSwitch === "1") {
              this.switchText = this.$t("smalltalk.open");
            } else {
              this.switchText = this.$t("smalltalk.close");
            }
    ...
    nodeClick(nodeId) {
      ...
      this.nodeId = nodeId.currentNodeId;
    ...
    delFaq(row, index, type) {
      ...
      if (type === "only") {
        ids.push(row.id);
        isOnlyStr = this.$t("smalltalk.should");
      } else {
        if (this.selectionList.length === 0) {
          this.$message({
            type: "warning",
            message: this.$t("smalltalk.selectKnowledgeTips"),
          });
          return false;
        }
        ...
        if (this.selectionList.length > 0 && this.isAudit ) {
          ...
          if( !canDel ){
            this.$message({
              type: "warning",
              message: this.$t("smalltalk.selectKnowledgeHasAudit"),
            });
            return false;
          }
        }
        ...
        isOnlyStr = this.$t("smalltalk.these");
      }
      this.$confirm(
        `${this.$t("smalltalk.deletionTips")} ${isOnlyStr} ${this.$t(
          "smalltalk.knowledge"
        )}？`,
        {
    ...
    enableOrStop(row, type, isBatch) {
      ...
      if (type === 1) {
        str = this.$t("smalltalk.enable");
      } else {
        str = this.$t("smalltalk.deactivate");
      }

      this.$confirm(
        `${this.$t("smalltalk.sureTips")} ${str}${isOnlyStr} ${this.$t(
          "smalltalk.knowledge"
        )}？`,
        {
          confirmButtonText: this.$t("aicosCommon.confirmBtn"),
    ...
    batchOperation(command) {
      if (command === "del") {
        ...
        form.setAttribute("action", `${baseURL}api/${knowledge}/gossip/export`);
        ...
        oneKeySync(this.curTreeNodeId).then((res) => {
          if (res.data.code == 200) {
            this.$message.success(this.$t("smalltalk.syncSuccessfulTips"));
            this.onLoad();
          }
        });
      }
    ...
    onLoad() {
      ...
      info = {
        ...
        cateId: this.nodeId ? this.nodeId : "0",
      ...
      if (
        ...
        // console.log(this.curTreeNodeId);
        getById(this.nodeId).then((res) => {
          if (res.data.success) {
            this.generateSwitch =
              res.data.data.isGenerativeDialogue === null
                ? "1"
                : res.data.data.isGenerativeDialogue;
            if (this.generateSwitch === "1") {
              this.switchText = this.$t("smalltalk.open");
            } else {
              this.switchText = this.$t("smalltalk.close");
