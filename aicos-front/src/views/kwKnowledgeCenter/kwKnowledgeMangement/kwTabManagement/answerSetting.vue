  ...
  <div class="kw-tabs-base-container answerSetting">
    ...
    >
          ...
          <div class="kw-button-container">
            ...
            <div>
              ...
              <!-- <el-button
                ...
                >{{ $t("table.syncKnowledge") }}</el-button
              > -->
              <el-button
                @click="handleDelete"
                size="small"
      ...
      <template slot-scope="scope" slot="menu">
        ...
        <el-button
          ...
          v-if="
            (!isAudit || rowData.auditStatus !== 'auditing') &&
            type !== 'version'
          "
        >
        ...
        <!-- <el-button
          v-if="scope.row.answerStatus === 0 && !isAudit"
          type="text"
          size="small"
          @click.stop="publishFaq(scope.row, 1)"
        >
          {{ $t("table.release") }}
        </el-button> -->
        <el-button
          v-if="scope.row.enabledState === 0 && type !== 'version'"
          type="text"
          size="small"
          @click.stop="changeStatus(scope.row, 1)"
        >
          {{ $t("table.enable") }}
        </el-button>
        <el-button
          v-else-if="scope.row.enabledState === 1 && type !== 'version'"
          type="text"
          size="small"
          @click.stop="changeStatus(scope.row, 0)"
        >
          {{ $t("table.deactivate") }}
        </el-button>
        <el-button
        ...
        <!-- <el-button
          type="text"
          size="small"
          @click.stop="openVersionRecordDialog(scope.row, scope.index)"
        >
          {{ $t("table.versionRecord") }}
        </el-button> -->
      </template>
    ...
    >
      <AnswerVersionRecord
        ...
        :rowData="rowData"
        :curVesionRecordRow="curVesionRecordRow"
      />
      <template v-slot:footer>
...
import AnswerVersionRecord from "./answerVersionRecord";
...
export default {
  ...
  methods: {
    handleSync() {
      oneKeySync(this.rowData.id).then((res) => {
    ...
    publishFaq(row) {
      ...
      // obj.answerStatus = status;
      publishFaqUrl(row.id).then((res) => {
        if (res.data.success) {
          this.$message.success(this.$t("table.publishedSuccessfully"));
          this.onLoad();
        }
      });
    },
    ...
    //删除
    handlerDel(row) {
      this.$confirm(this.$t("table.deleteDocumentTips"), {
    ...
    handleDelete() {
      if (this.idsArr.length === 0) {
        this.$message.warning(this.$t("table.selectKnowledgeTips"));
        return false;
      }
      this.$confirm(this.$t("table.deleteKnowledgeTips"), {
        confirmButtonText: this.$t("aicosCommon.confirmBtn"),
        cancelButtonText: this.$t("aicosCommon.cancelBtn"),
        type: "warning",
      })
    ...
    rowEdit(row) {
      this.selectRow = row;
      this.dialogTitle = this.$t("aicosCommon.editBtn");
      this.addFlag = true;
    },
    ...
    //新增/编辑
    handlerSave(status) {
      const form = this.$refs.addanswer.$refs.form;
      form.validate((validate, done) => {
        if (validate) {
          const data = this.$refs.addanswer.submit();
