<template>
  <HuilanBasic class="tabKnowledge" :left-width="250">
          ...
          <div class="faq-search-header-wrap clearfix">
            <div class="faq-operation-btns">
              <el-button
                ...
                >{{ $t("table.addKnowledge") }}</el-button
              >
              <el-dropdown
                v-if="
                  permission[`${prefix}table-startMore`] ||
                  permission[`${prefix}table-stopMore`] ||
                  permission[`${prefix}table-deleteMore`]
                "
              ...
              >
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    command="enable"
                    v-if="permission[`${prefix}table-startMore`]"
                    >{{ $t("aicosCommon.batchStart") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="stop"
                    v-if="permission[`${prefix}table-stopMore`]"
                    >{{ $t("aicosCommon.batchPause") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="del"
                    v-if="permission[`${prefix}table-deleteMore`]"
                    >{{ $t("aicosCommon.batchDeleteBtn") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="batchExport"
                    v-if="
                      permission[`${prefix}table-batchExport`] &&
                      curTreeNodeId != '0' &&
                      curTreeNodeId != 'mine' &&
                      curTreeNodeId != ''
                    "
                  ...
                  <el-dropdown-item
                    command="batchImport"
                    v-if="permission[`${prefix}table-batchImport`]"
                    >{{ $t("aicosCommon.batchImport") }}</el-dropdown-item
                  >
                  <el-dropdown-item
                    command="handleSync"
                    v-if="permission[`${prefix}table-sync`]"
                  >
                    {{ $t("table.syncKnowledge") }}
                  </el-dropdown-item>
                  <!-- <el-dropdown-item command="import">批量导出</el-dropdown-item>
              ...
              <div class="faq-search-input-row">
                ...
                <div class="faq-search-value">
                  <div class="faq-search-input">
                    <el-input
                      :placeholder="
                        $t('aicosCommon.pleaseInput') + $t('table.keywords')
                      "
                      size="small"
                ...
                <dd class="dismension-more-dd-wrap clearfix">
                  <div class="search-other-item">
                    <el-select
                      ...
                      v-model="queryObj.knowledgeStatus"
                      :placeholder="$t('table.selectKwStatus')"
                    >
                      <el-option
                  ...
                  <div class="search-other-item">
                    <el-select
                      size="small"
                      clearable="true"
                      @change="searchStatusChange('availableStatus')"
                      @clear="clearSearchChange('availableStatus')"
                      v-model="queryObj.availableStatus"
                      :placeholder="
                        $t('aicosCommon.pleaseSelect') +
                        $t('table.periodValidity')
                      "
                    >
                      <el-option
                  ...
                  <div class="search-other-item">
                    <el-select
                      ...
                      :placeholder="
                        $t('aicosCommon.pleaseSelect') + $t('table.kwStatus')
                      "
                    >
                      <el-option
    ...
    <el-drawer
      ...
      append-to-body
      show-close
      @close="(drawerVisible = false), (faqManageType = 'mangelist')"
      :size="faqManageType === 'answer' ? '80%' : '40%'"
    >
      ...
      <template v-if="faqManageType == 'edit' || faqManageType == 'view'">
        <AddFaq
          :nodeDeepPath="curNodeDeepPath"
          :curTreeNode="curSelectedTreeNode"
          :curFaqObj="curVesionRecordRow"
          :faqType="faqManageType"
          @addOrEditEmitEvent="addOrEditEmitEvent"
        />
      </template>
      ...
      <template v-if="faqManageType === 'answer'">
        <AnswerSetting
    ...
    >
      <TreeSetting
        ...
        :classType="classType"
        v-if="settingDrawer"
        :treeRootName="$t('doc.generalClass')"
      />
...
// import Import from "./kwImportComponent/index";
...
import { oneKeySync } from "@/api/kwKnowledgeCenter/kwKnowledgeMangement/kwTabManagement/answersetting";
  ...
  data() {
    return {
      ...
      searchOptions: [
        ...
        {
          label: this.$t("table.similarProblems"),
          value: "3",
        },
      ],
      searchStatusOptions: [
      ...
      searchAvaliableStatusOptions: [
        ...
        {
          label: this.$t("table.notLongTerm"),
          value: "1",
        },
        // {
        //   label: "已失效",
      ...
      faqTableOption: {
        ...
        column: [
          ...
          {
            label: this.$t("table.auditStatus"),
            prop: "auditStatus",
            span: 12,
            minWidth: 100,
            showWordLimit: true,
            ...
            dicData: [
              ...
              {
                label: this.$t("table.reject"),
                value: "reject",
              },
              {
                label: this.$t("table.disagree"),
                value: "disagree",
              },
              {
                label: this.$t("table.finish"),
                value: "finish",
              },
              {
                label: this.$t("table.withdraw"),
                value: "withdraw",
              },
              // {
              //   label: "通过",
    ...
    permissionList() {
      return {
        addBtn: this.vaildData(
          this.permission[`${this.prefix}table-add`],
          false
        ),
        viewBtn: this.vaildData(
          this.permission[`${this.prefix}table-view`],
          false
        ),
        editBtn: this.vaildData(
          this.permission[`${this.prefix}table-edit`],
          false
        ),
        delBtn: this.vaildData(
          this.permission[`${this.prefix}table-delete`],
          false
        ),
        answerBtn: this.vaildData(
          this.permission[`${this.prefix}table-answersetting`],
          false
        ),
        publishBtn: this.vaildData(
          this.permission[`${this.prefix}table-publish`],
          false
        ),
        stopBtn: this.vaildData(
          this.permission[`${this.prefix}table-stop`],
          false
        ),
        startBtn: this.vaildData(
          this.permission[`${this.prefix}table-start`],
          false
        ),
        versionBtn: this.vaildData(
          this.permission[`${this.prefix}table-colophon`],
          false
        ),
        topBtn: this.vaildData(
          this.permission[`${this.prefix}table-top`],
          false
        ),
        cancelTopBtn: this.vaildData(
          this.permission[`${this.prefix}table-canceltop`],
          false
        ),
        viewRecodeBtn: true,
        closeAuditBtn: true,
  ...
  methods: {
    ...
    getRowActions(row) {
      const map = {
        ...
        // },
        answerBtn: {
          name: this.$t("table.answerConfiguration"),
          command: "answerSettingClick",
        },
        // versionBtn: {
        //   name: "版本记录",
      ...
      if (row.auditStatus == "auditing" && this.isAudit) {
        map.closeAuditBtn = {
          name: this.$t("table.withdraw"),
          command: "closeAudit",
        };
        list.push("closeAuditBtn");
      }
      if (row.knowledgeStatus == 0 && this.validatenull(row.auditStatus)) {
        map.publishBtn = {
          name: this.$t("table.release"),
          command: "publishFaq",
        };
      ...
      if (row.questionOrder == "1") {
        map.cancelTopBtn = {
          name: this.$t("table.cancelTopping"),
          command: "updateTopStatus",
        };
        list.push("cancelTopBtn");
      }
      if (
        this.isAudit
        // row.auditStatus == "auditing" ||
        ...
        map.viewRecodeBtn = {
          name: this.$t("table.auditRecords"),
      ...
      if (
        ...
        map.versionBtn = {
          name: this.$t("table.versionRecord"),
          command: "openVersionRecordDialog",
        };
        list.push("versionBtn");
      }
      const actions = [];
    ...
    handleCommand(command, row, index) {
      if (command === "enableOrStop") {
        this.enableOrStop(row, row.status == 0 ? 1 : 0, "only");
    ...
    // 答案配置
    answerSettingClick(row) {
      this.selectRow = row;
      this.faqManageType = "answer";
      this.drawerTitle = this.$t("table.answerConfiguration");
    ...
    sureSearchDismension() {
      ...
      if (dialogDismensionData.length == 1) {
        this.$message({
          message: this.$t("table.secondaryTips"),
          type: "warning",
        });
        return false;
      }
    ...
    openFaqAddDialog() {
      ...
      this.drawerVisible = true;
      this.drawerTitle = this.$t("aicosCommon.addBtn");
    },
    ...
    viewFaq(row) {
      ...
      this.drawerTitle = this.$t("aicosCommon.viewBtn");
      this.drawerVisible = true;
    },
    ...
    openEditFaq(row) {
      ...
      this.drawerTitle = this.$t("aicosCommon.editBtn");
      this.drawerVisible = true;
    },
    ...
    delFaq(row, index, type) {
      ...
      if (type == "only") {
        ids.push(row.id);
        isOnlyStr = this.$t("table.should");
      } else {
        if (this.selectionList.length == 0) {
          this.$message({
            type: "warning",
            message: this.$t("table.selectKnowledgeTips"),
          });
          return false;
        }
        ...
        if (this.selectionList.length > 0 && this.isAudit ) {
          ...
          if( !canDel ){
            this.$message({
              type: "warning",
              message: this.$t("table.selectKnowledgeHasAudit"),
            });
            return false;
          }
        }
        ...
        isOnlyStr = this.$t("table.these");
      }
      this.$confirm(
        `${this.$t("table.deletionTips")} ${isOnlyStr} ${this.$t(
          "table.knowledge"
        )}？`,
        {
    ...
    enableOrStop(row, type, isBatch) {
      ...
      if (type === 1) {
        str = this.$t("table.enable");
      } else {
        str = this.$t("table.deactivate");
      }

      this.$confirm(
        `${this.$t("table.sureTips")} ${str} ${isOnlyStr} ${this.$t(
          "table.knowledge"
        )}？`,
        {
          confirmButtonText: this.$t("aicosCommon.confirmBtn"),
        ...
        .then(() => {
          this.$message({
            type: "success",
            message: `${isOnlyStr}${this.$t(
              "table.knowledge"
            )} ${str} ${this.$t("table.success")}！`,
          });
          this.onLoad();
        });
    },
    ...
    batchOperation(command) {
      if (command == "del") {
        ...
        avaliableStatusInput.setAttribute("name", "availableStatus");
        avaliableStatusInput.setAttribute(
          "value",
          this.queryObj.availableStatus
        );
        ...
        statusInput.setAttribute("type", "hidden");
        statusInput.setAttribute("name", "enable");
        statusInput.setAttribute("value", this.queryObj.enable);
        form.append(statusInput);

        form.setAttribute(
          "action",
          `${baseURL}api/${knowledge}/tableQa/export`
        );
      ...
      } else if (command == "batchExport") {
        const dataParam = {
          question:
            this.searchObj.titleType === "1" ? this.searchObj.content : "",
          similarQuestion:
            this.searchObj.titleType === "3" ? this.searchObj.content : "",
          botId: this.botObj.id,
          cateId: this.curTreeNodeId ? this.curTreeNodeId : "0",
        };

        tableBatchExport(dataParam).then(
          (res) => {
            ...
            this.downFile(resData, fileNameDecode);
    ...
    handleSync() {
      ...
      oneKeySync(ids.join(",")).then((res) => {
        if (res.data.code == 200) {
          this.$message.success(this.$t("table.syncSuccessfulTips"));
          this.onLoad();
        }
      });
    },
