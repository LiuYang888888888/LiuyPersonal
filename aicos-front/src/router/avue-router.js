    ...
    formatRoutes: function (aMenu = [], first) {
      ...
      for (let i = 0; i < aMenu.length; i++) {
        ...
        let path = (() => {
          ...
          component = oMenu[propsDefault.path].replace(/^\//, ''),
          // name = oMenu[propsDefault.label],
          // name = this.generateTitle(oMenu[propsDefault.label], oMenu.code),
          name = oMenu.code,
        ...
        // console.log(component)
        meta = Object.assign(meta, {i18n: oMenu.code}, (function () {
        ...
        // 远程组件
        let remoteComponent
        if(oMenu.moduleRemote === 1) {
          remoteComponent = createRemoteComponent(oMenu)
        }
        //是否有子路由
        ...
        const oRouter = {
          ...
          redirect: (() => {
            ...
            if (!isChild && first && !isURL(path)) return path.endsWith('/index')? path : `${path}/index`
            else return '';
          })(),
          // 整理子路由的route 配置
          // 处理是否为一级路由
          children: !isChild ? (() => {
            if (first) {
              ...
              if (!isURL(path)) oMenu[propsDefault.path] = path.endsWith('/index')? path : `${path}/index`;
              return [{
                component(resolve) {
                  if(oMenu.moduleRemote === 1) {
                    resolve(remoteComponent)
                  }else {
                    require([`../views/${component}.vue`], function(res) {
                      resolve(res)
                    },function(err) {
                      if(err.code === 'MODULE_NOT_FOUND') {
                        require([`../views/${component.replace('/index', '')}.vue`], resolve)
                      }
                    })
                  }
