  ...
  <HuilanBasic>
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.delBtn && scope.row.extendField8 != '2'"
                   @click="handelDel(scope.row)">删除
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.examineBtn && scope.row.extendField8 != '2'"
...
export default {
  ...
  methods: {
    ...
    handleView(row, index) {
      if(row.extendField1 == 1){
        console.log(1111111111)
        this.option.column[8].disabled = true
        console.log(this.option.column[8].disabled)
