    ...
    >
      <template slot-scope="scope" slot="menuRight">
        <el-button
          ...
          size="small"
          icon="el-icon-plus"
          v-if="permissionList.addMagBtn && metObjName !== 'dyf_aicos_gongwykhzzbxftz'"
        ...
        <el-button
          ...
          size="small"
          icon="el-icon-plus"
          v-if="permissionList.jiaBtn && metObjName == 'dyf_aicos_gongwykhzzbxftz'"
...
export default {
    ...
    permissionList() {
      return {
        ...
        tijiaoBtn: this.vaildData(
          this.permission[`sys-commonForm-dataMag-tijiao`],
          false
        ),
        ...
        shangbaoBtn: this.vaildData(
          this.permission[`sys-commonForm-dataMag-shangbao`],
          false
        ),
      };
    },
  ...
  data() {
    return {
      ...
      yueduTitle:'阅读情况',
      yueduShow:false,
      tableDatayuedu:[{chushi:'秘书处',yueduzt:'已读'}],

      xinxiTitle:'信息收集',
      xinxiShow:false,
      tableDataxinxi:[{title:'上报收集信息',part:'秘书处',zt:'已提交',people:'测试',date:'2024-05-06'}],
  ...
  methods: {
    getRowActions() {
      ...
      if(this.metObjName == 'dyf_aicos_gongwykhzzbxftz'){
      ...
      } else if(this.metObjName == 'dyf_aicos_renwgltzxflb'){
        if (this.permissionList.tzxiafaBtn) {
          map.tzxiafaBtn = {
            name: "下发",
            command: "comFormMagTzxiafa",
          };
          list.push("tzxiafaBtn");
        }
        if (this.permissionList.yueduBtn) {
          map.yueduBtn = {
            name: "阅读情况",
            command: "comFormMagYuedu",
          };
          list.push("yueduBtn");
        }
        if (this.permissionList.xinxiBtn) {
          map.xinxiBtn = {
            name: "信息收集",
            command: "comFormMagXinxi",
          };
          list.push("xinxiBtn");
        }
        ...
        if(this.metObjName == 'dyf_aicos_chusrykhgl'){
          if(this.permissionList.tijiaoBtn){
            map.tijiaoBtn = {
              name: "提交",
              command: "comFormMagTijiao",
            };
            list.unshift("tijiaoBtn");
          }
        }
        if(this.metObjName == 'dyf_aicos_gecstbrwlb'){
          if(this.permissionList.shangbaoBtn){
            map.shangbaoBtn = {
              name: "上报",
              command: "comFormMagShangbao",
            };
            list.unshift("shangbaoBtn");
          }
        }
        if ((this.formVerify && this.tabType != "TODO") || !this.formVerify) {
          map.viewMagBtn = {
    ...
    async comFormMagView(row /*, index*/) {
      if (!validatenull(this.customViewUrl)) {
        ...
        let openUrl = this.customViewUrl.includes("?")
          ...
          : this.customViewUrl + "?" + zUrl;
        window.open(openUrl);
      } else if(this.metObjName == 'dyf_aicos_chusllytjbcshzb'){
        this.btnLoading = false;
        this.hzDetail = true
      }else {
        this.verifyTab = "VIEW";
    ...
    searchMagFormReset() {
      ...
      this.$refs.searchMagFormRef.resetForm();dataMagFormSave
      this.query = {};
      this.page.currentPage = 1;
      this.onLoad(this.page);
    },
