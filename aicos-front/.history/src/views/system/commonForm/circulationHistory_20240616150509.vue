...
export default {
  ...
  methods: {
    ...
    function hexToStr(hexString) { // 移除 \x 并将十六进制字符串转换为字节数组 
      let hex = hexString.replace(/\\x/g, ''); let str = ''; for (let i = 0; i < hex.length; i += 2) { str += String.fromCharCode(parseInt(hex.substr(i, 2), 16)); } return str; }
