...
export default {
  ...
  methods: {
    ...
    function hexToUtf8Str(hexString) {
    // 移除 \x 并将十六进制字符串转换为字节数组
    ...
    let utf8Array = new Uint8Array(bytes);
    let str = decoder.decode(utf8Array);
    ...
    async getHistoryList() {
      ...
      res.forEach(e => {
        if(e.FlowVO.comment){
          e.FlowVO.comment = this.hexToStr(e.FlowVO.comment)
        }
      });
