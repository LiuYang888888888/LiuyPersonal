  ...
  <basic-container>
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="scope.row.shiftj == '否' && (role.includes('处长') < 0 || role.includes('政治部') < 0)"
          @click="handleUpdate(scope.row,scope.index)">编辑
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.kqtjBtn && scope.row.shiftj == '否' && (role.includes('处长') < 0 || role.includes('政治部') < 0)"
          @click="comFormMagkqtj(scope.row)">提交
        </el-button>
        <el-button type="text" size="small" v-if="scope.row.shiftj == '否' && role.includes('处长') > 0"
