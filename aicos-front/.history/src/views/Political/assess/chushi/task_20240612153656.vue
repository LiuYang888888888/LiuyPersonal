...
import { getList, add, update, remove, tijiao, role ,getprocessId,getinstanceId} from "@/api/Political/common";
import {
  ...
  methods: {
    ...
    handlezt(row){
      getprocessId({id:row.id}).then((res) => {
        ...
        getinstanceId({processInstanceId:res.data.data[0].process_instance_id}).then((res) =>{
          console.log(res)
        })
      })
    },
