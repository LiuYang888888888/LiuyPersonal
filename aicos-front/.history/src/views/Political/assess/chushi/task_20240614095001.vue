  ...
  <basic-container>
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="scope.row.shiftj == '草稿' && role"
          @click="handleUpdate(scope.row, scope.index)">编辑
        </el-button>
        <el-button type="text" size="small" v-if="role && scope.row.shiftj != '草稿'"
          @click="handlezt(scope.row, scope.index)">流程
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.kqtjBtn && role && scope.row.shiftj == '草稿'" @click="comFormMagkqtj(scope.row)">提交
        </el-button>
        <el-button type="text" size="small" v-if="scope.row.shiftj == '草稿' && role" @click="handelDel(scope.row)">删除
        </el-button>
    ...
    <HuilanDialog :title="dataMagFormTitle" :fullscreen="true" append-to-body :before-close="dataMagFormClose"
      ...
      <div class="dataMagFormBody" v-if="verifyTab == 'VIEW'">
        <div class="CMF_MAIN">
          ...
          <div class="CMF_BODY">
            <avue-form id="dataMagFormView" ref="dataMagFormRef" v-model="form2" :option="option2" v-if="!role2 && tabType == 'TODO'">
            </avue-form>
            <avue-form id="dataMagFormView2" ref="dataMagFormRef2" v-model="form3" :option="option3" v-if="role2 && tabType == 'TODO'">
            </avue-form>
        ...
        <div class="dataMagFormFooter">
          ...
          <div class="dataMagFormBtns">
            ...
            <el-button size="small" icon="el-icon-circle-close" @click="dataMagFormClose">取 消
            </el-button>
          </div>
        </div>
      </div>
...
import { getList, add, update, remove, tijiao, role ,getprocessId,flowNo,flowYes} from "@/api/Political/common";
import {
...
export default {
  ...
  methods: {
    ...
    async dataMagVerifyYes () {
      if (this.role2) {
        this.$refs.dataMagFormRef2.validate((valid /*msg*/) => {
          if (valid) {
            this.btnLoading = true;
            this.form3.zhengzbsdrq = this.zhengzhidate
            this.form3.zhengzbsd = this.zhengzhival
            const dataParam = {
              formTemplateVersionId:this.templateId,
              flowVO:{
                ...this.verifyParamData,
                passStatus: "agree",
    ...
    comFormMagkqtj (row) {
      this.$confirm("确认提交?", {
        ...
        .then(() => {
          return tijiao({formId:row.id,formTemplateVersionId:this.templateId})
        })
        .then(() => {
