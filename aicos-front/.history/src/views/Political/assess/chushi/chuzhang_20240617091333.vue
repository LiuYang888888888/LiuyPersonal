  ...
  <basic-container>
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.kqtjBtn && role && (scope.row.shiftj == '草稿' || scope.row.shiftj == '审核不通过')" @click="comFormMagkqtj(scope.row)">提交
        </el-button>
        <el-button type="text" size="small" v-if="(scope.row.shiftj == '草稿' || scope.row.shiftj == '审核不通过') && role" @click="handelDel(scope.row)">删除
        </el-button>
...
export default {
  ...
  methods: {
    ...
    async comFormMagVerify (row) {
      ...
      this.dataMagFormKey++;
      if (this.rolename.includes("分管领导")) {
    ...
    comFormMagkqtj (row) {
      this.$confirm("确认提交?", {
        ...
        .then(() => {
          return cztijiao({formId:row.id,formTemplateVersionId:this.templateId})
        })
        .then(() => {
