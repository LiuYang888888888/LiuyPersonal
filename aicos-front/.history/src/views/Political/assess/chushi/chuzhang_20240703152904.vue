  ...
  <basic-container>
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        <el-button type="text" size="small" @click="handleView(scope.row, scope.index)" v-if="tabType != 'TODO'">查看
        </el-button>
    ...
    <HuilanDialog :title="dataMagFormTitle" :fullscreen="true" append-to-body :before-close="dataMagFormClose"
      ...
      <div class="dataMagFormBody" v-if="verifyTab == 'VIEW'">
        <div class="CMF_MAIN">
          ...
          <div class="CMF_BODY">
            <avue-form id="dataMagFormView" ref="dataMagFormRef" v-model="form2" :option="option2" v-if="tabType == 'TODO' && rolefinal == 0">
            </avue-form>
            <avue-form id="dataMagFormView3" ref="dataMagFormRef3" v-model="form3" :option="option3" v-if="tabType == 'TODO' && rolefinal == 3">
            </avue-form>
            <avue-form id="dataMagFormView4" ref="dataMagFormRef4" v-model="form4" :option="option4" v-if="tabType == 'DONE'">
            </avue-form>
          </div>
        </div>
...
import { czgetList, saveOrUpdateListData, viewListData, czremove, cztijiao, role ,getprocessId,czflowNo,czflowYes,userInfo,getDepartment} from "@/api/Political/common";
import {
...
export default {
  ...
  methods: {
    ...
    async comFormMagVerify (row) {
      ...
      if(this.rolefinal == 3){
        this.form3 = await this.formData(row);
      }else{
        this.form2 = await this.formData(row);
      }
    ...
    async dataMagVerifyYes () {
      this.$refs.dataMagFormRef.validate((valid /*msg*/) => {
        if (valid) {
          ...
          if(this.rolefinal == 3){
            this.form3.zhengzbsd = this.zhengzhival
            this.form3.zhengzbsdrq = this.zhengzhidate
            saveOrUpdateListData(this.form3).then(
              (/*res*/) => {
                const dataParam2 = {
                  ...this.verifyParamData,
                  passStatus: "agree",
            ...
            this.form2.zhugldpj = this.pingjianval
            this.form2.qianm = this.pingjianqianming
            this.form2.pingjrq = this.pingjiandate
            saveOrUpdateListData(this.form2).then(
              (/*res*/) => {
                const dataParam2 = {
                  ...this.verifyParamData,
                  passStatus: "agree",
    ...
    async handleView (row, index) {
      if(this.tabType == 'ALL'){
        ...
        // await this.getTemplateVersion(this.dataInitTemplateId);
        // this.dataFormData = await this.comFormMagDataInit(row);
        this.form4 = await this.formData(row);
        this.dataMagFormSubBtnTag = false;
      }
    },
    ...
    async beforeOpen (done, type) {
      if (["add", "edit", "view"].includes(type)) {
        if(type == "add"){
          ...
          this.form.suosbm = dept.data.data.deptName
        }
        this.initData(type);
      }
      done();
