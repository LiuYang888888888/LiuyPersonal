    ...
    async dataMagVerifyYes(done, loading) {
      this.$refs.dataMagFormRef.validate((valid /*msg*/) => {
        if (valid) {
          ...
          console.log(this.from2)
          this.from2.pingjrq = this.pingjiandate
          this.from2.zhugldpj = this.pingjianval
          this.from2.qianm = this.pingjianqianming
          flowYes(dataParam).then(
            (/*res*/) => {
              update(this.from2).then(() => {
                done();
