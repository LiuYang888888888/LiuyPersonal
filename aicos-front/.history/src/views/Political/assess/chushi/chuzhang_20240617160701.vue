<template>
  <basic-container>
    <el-tabs v-if="formVerify && !role && role2 == 2" v-model="tabType" @tab-click="tabChange">
      <el-tab-pane name="ALL" v-if="">
    ...
    <!-- <el-tabs v-if="formVerify && role2 == 0" v-model="tabType2" @tab-click="tabChange2">
      <el-tab-pane name="TODO" v-if="permissionList.toDoBtn">
        <span slot="label"><i class="el-icon-s-management"></i> 待审核</span>
      </el-tab-pane>
      <el-tab-pane name="DONE" v-if="permissionList.doneBtn">
    ...
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        <el-button type="text" size="small" @click="handleView(scope.row, scope.index)" v-if="tabType != 'TODO' && tabType2 !='TODO'">查看
        </el-button>
        <el-button type="text" size="small" v-if="(scope.row.shiftj == '草稿' || scope.row.shiftj == '审核不通过') && (role2 == scope.row.rylx) "
          @click="handleUpdate(scope.row, scope.index)">编辑
        </el-button>
        <el-button type="text" size="small" v-if="role2 == scope.row.rylx"
        ...
        <el-button type="text" size="small" v-if="(scope.row.shiftj == '草稿' || scope.row.shiftj == '审核不通过') && (role2 == scope.row.rylx)" @click="handelDel(scope.row)">删除
        </el-button>
        <el-button type="text" size="small" v-if="formVerify && (tabType == 'TODO' || tabType2 == 'TODO')"
    ...
    <HuilanDialog :title="dataMagFormTitle" :fullscreen="true" append-to-body :before-close="dataMagFormClose"
      ...
      <el-tabs v-if="formVerify && ((tabType == 'TODO' || tabType == 'DONE') || (tabType2 == 'TODO' || tabType2 == 'DONE'))" v-model="verifyTab"
      ...
      <div class="dataMagFormBody" v-if="verifyTab == 'VIEW'">
        ...
        <div class="dataMagFormFooter">
          <div v-if="formVerify && !role && (tabType == 'TODO' || tabType2 =='TODO')" class="formVerifyClass">
            <el-input type="textarea" v-model="formVerifyText" placeholder="请输入审核意见" rows="4" />
          </div>
          <div class="dataMagFormBtns">
            <el-button size="small" icon="el-icon-check" v-if="formVerify && !role && (tabType == 'TODO' || tabType2 =='TODO')" type="success" @click.stop="dataMagVerifyYes" :loading="btnLoading">审核通过
            </el-button>
            <el-button size="small" icon="el-icon-close"
              v-if="formVerify && !role && (tabType == 'TODO' || tabType2 =='TODO')" type="warning"
...
import { czgetList, saveOrUpdateListData, update, czremove, cztijiao, role ,getprocessId,czflowNo,czflowYes} from "@/api/Political/common";
import {
...
export default {
  ...
  methods: {
    ...
    async comFormMagVerify (row) {
      ...
      this.dataMagFormKey++;
      if (this.rolename.includes("分管领导") || this.rolename.includes("处长")) {
        this.comFormMagItem("shenhe1");
      } else {
        this.comFormMagItem("shenhe2");
      }
    ...
    async roleInfo () {
      ...
      if (rolename.includes("处长")) {
        this.role2 = 2
        this.tabType2 = "ALL"
      }else if(rolename.includes("政治部") || rolename.includes("分管领导")){
        this.tabType2 = "TODO"
      }
    ...
    async handleView (row, index) {
      if(this.tabType == 'ALL' && this.tabType2 == 'ALL'){
    ...
    onLoad (page) {
      ...
      if (this.tabType == "TODO" || this.tabType2 == "TODO") {
        const dataParam = {
      ...
      } else if (this.tabType == "DONE" || this.tabType2 == "DONE") {
        const dataParam = {
          metaObjName: this.metObjName,
          templateVersionId: this.templateId,
          systemCode: this.systemCode,
    ...
    comFormMagItem(actionType) {
      ...
      formItems.map((item,i) => {
        if (actionType == "view") {
          item.disabled = true;
        }else if(actionType == "shenhe1"){
          if(i == 3){
            item.data.map( (item) =>{
              if(item.length > 0){
                item.map( (item) =>{
                  let availableStateFlag = item.children.some(item => item.prop);
                  if(availableStateFlag){
                    if(item.children[0].prop == 'string_zhugldpj' || item.children[0].prop == 'string_qianm' || item.children[0].prop == 'datetime_pingjrq'){
            ...
            item.data.map( (item) =>{
              if(item.length > 0){
                item.map( (item) =>{
                  let availableStateFlag = item.children.some(item => item.prop);
                  if(availableStateFlag){
                    if(item.children[0].prop == 'string_zhengzbsd' || item.children[0].prop == 'datetime_zzbsdrq'){
                      item.children[0].disabled = false;
                    }else{
                      item.children[0].disabled = true
                    }
          ...
          if(i == 3){
            item.data.map( (item) =>{
              if(item.length > 0){
                item.map( (item) =>{
                  let availableStateFlag = item.children.some(item => item.prop);
                  if(availableStateFlag){
                    if(item.children[0].prop == 'string_zhugldpj' || item.children[0].prop == 'string_qianm' 
                    || item.children[0].prop == 'datetime_pingjrq' || item.children[0].prop == 'string_zhengzbsd' || item.children[0].prop == 'datetime_zzbsdrq'){
                      item.children[0].disabled = true;
                    }else{
                      item.children[0].disabled = false
                    }
            ...
            if(item.prop == "datetime_tianbny1"){
              item.type = 'month'
            }
            item.disabled = false;
          }
