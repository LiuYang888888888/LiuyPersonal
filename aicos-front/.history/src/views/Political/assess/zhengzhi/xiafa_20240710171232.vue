<template>
  <basic-container>
    ...
    <!--各部门明细表汇总-->
    <HuilanDialog :title="mxhzTitle" :fullscreen="true" width="80%" append-to-body :visible.sync="mzhxDetail">
    ...
    <!--各部门明细详情-->
    <HuilanDialog :title="bumenTitle" :fullscreen="true" width="80%" append-to-body :visible.sync="bumenDetail">
      <div style="width: 100%;padding:10px">
        <div class="jidumain">
          <div class="chushi">处室：{{ bumen }}</div>
          <div class="jidu">{{ jidu }}</div>
        </div>
        <table align="center" width="100%" class="table xunw_table_form" border="1">
          <tr>
            <td>序号</td>
      ...
      kaoheDetailSumDept({chusdm:row.deptid,guanlzbid:row.id}).then(res =>{
        if(res.data.code == 200){
          ...
          this.bumenList.forEach((item) => {
            console.log(if(item.this.monthall1){
