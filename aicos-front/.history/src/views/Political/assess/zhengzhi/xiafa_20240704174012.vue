  ...
  <basic-container>
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" @click="comFormMagJia(scope.row)">生成
        </el-button>
        <el-button type="text" size="small" @click="handleEdit(scope.row)">编辑
        </el-button>
        <el-button type="text" size="small" @click="handleXiafa(scope.row)">下发
        </el-button>
    ...
    comFormMagDetail (row) {
      getDepartment().then(res =>{
        if(res.data.data.deptName.includes('政治部')){
          this.kaoqinTag = true
          getNoticeDetail({guanlzbid:row.id,current:1,size:9999,chusdm:}).then(res => {
            if (res.data.code == 200) {
              this.editid = false
              this.kqtitle = row.fabbt
              this.tableData = res.data.data
