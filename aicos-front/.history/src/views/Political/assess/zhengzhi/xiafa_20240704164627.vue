  ...
  <basic-container>
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.delBtn" @click="handleXiafa(scope.row)">删除
        </el-button>
...
import {getNotice,saveNotice,removeNotice,saveNoticeDetail,getNoticeDetail,saveNoticeDetailList,updateNotice,sendNotice} from "@/api/Political/common";
export default {
  data () {
    return {
      kaoqinTitle: '下发通知',
  ...
  methods: {
    ...
    comFormMagDetail (row) {
      this.kaoqinTag = true
      getNoticeDetail({guanlzbid:row.id}).then(res => {
        if (res.data.code == 200) {
          this.editid = false
          this.kqtitle = row.fabbt
          this.tableData = res.data.data
