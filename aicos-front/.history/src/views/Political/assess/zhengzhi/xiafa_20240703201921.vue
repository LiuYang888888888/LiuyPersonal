  ...
  <basic-container>
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.detailBtn" @click="comFormMagJia(scope.row)">生成
        </el-button>
        ...
        <el-button type="text" size="small" v-if="permissionList.huizongBtn" @click="handleHuizong(scope.row)">各部门汇总明细
        </el-button>
    ...
    <!--考勤表单下发新增-->
    <HuilanDialog :title="kaoqinTitle" :fullscreen="false" width="90%" append-to-body :visible.sync="kaoqinTag">
      <div class="kqtitle">
      ...
      <el-table :data="tableData" height="300" border style="width: 60%;margin: 0 auto;">
        <el-table-column align="center" label="处室名称" width="180">
          <template slot-scope="{ row }">
            <span v-show="!row.editid">{{ row.chusmc }}</span>
            <el-input v-show="row.editid" v-model="row.chusmc"></el-input>
          </template>
        </el-table-column>

        <el-table-column align="center" label="月份">
          <template slot-scope="{ row }">
            <span v-show="!row.editid">{{ row.yuef }}</span>
            <el-input v-show="row.editid" v-model="row.yuef"></el-input>
          </template>
        </el-table-column>

        <el-table-column align="center" label="考核人数">
          <template slot-scope="{ row }">
            <span v-show="!row.editid">{{ row.kaohrs }}</span>
            <el-input v-show="row.editid" v-model="row.kaohrs"></el-input>
          </template>
        </el-table-column>

        <el-table-column align="center" label='“好”人数'>
          <template slot-scope="{ row }">
            <span v-show="!row.editid">{{ row.haors }}</span>
            <el-input v-show="row.editid" v-model="row.haors"></el-input>
          </template>
        </el-table-column>

        <el-table-column v-show="!kqdis" align="center" width="200" label="备注">
          <template slot-scope="{ row }">
            <span v-show="!row.editid">{{ row.beiz }}</span>
            <el-input v-show="row.editid" v-model="row.beiz" style="text-align: center;"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="{row}">
            ...
            <el-button type="primary" size="mini" v-show="kqdis" @click="kaoqinInfo(row)">
              详情
            </el-button>
          </template>
        </el-table-column>
    ...
    <!--处室详情表-->
    <HuilanDialog :title="kaoqinTitle" :fullscreen="false" width="80%" append-to-body :visible.sync="csInfo">
      <div class="kqtitle">
        <div class="kqtxt">标题：</div>
        <div>{{ kqtitle }}</div>
      </div>
    ...
    <!--处室个人考勤详情-->
    <HuilanDialog :title="csTitle" :fullscreen="false" width="80%" append-to-body :visible.sync="chushiDetail">
...
import {getNotice,saveNotice,saveNoticeDetail} from "@/api/Political/common";
export default {
  data () {
    return {
      kaoqinTitle: '下发通知',
