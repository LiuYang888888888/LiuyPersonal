    ...
    <avue-crud :option="optionyue" :table-loading="loadingyue" :data="datayue" :page="pageyue" :permission="permissionList"
      ...
      @current-change="currentChangeyue" @size-change="sizeChangeyue" @on-load="onLoadyue">
        <template v-slot:menu="scope">
          ...
          <el-button type="text" size="small" v-if="permissionList.yueviewBtn" @click="yueUpdate(scope.row,scope.index)">编辑
          </el-button>
        </template>
      </avue-crud>
  </basic-container>
    ...
    permissionList () {
      return {
        ...
        yueeditBtn: this.vaildData(
          this.permission[`sys-commonForm-dataMag-xf-yueedit`],
          false
        ),
      };
    },
