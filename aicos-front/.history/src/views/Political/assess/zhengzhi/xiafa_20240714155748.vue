  ...
  <basic-container>
    <avue-crud :option="option" :table-loading="loading" :data="data" :page="page" :permission="permissionList"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.yueBtn" @click="comFormMagYue(scope.row)">生成余额
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.jiaBtn" @click="comFormMagJia(scope.row)">生成明细
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.editMagBtn&& scope.row.fabzt == 0" @click="handleEdit(scope.row)">编辑
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.xiafaBtn&& scope.row.fabzt == 0" @click="handleXiafa(scope.row)">下发
        </el-button>
        <el-button type="text" size="small" v-if="permissionList.delMagBtn&& scope.row.fabzt == 0" @click="handledel(scope.row)">删除
        </el-button>
    ...
    <HuilanDialog :title="kaoqinTitle" :fullscreen="true" width="90%" append-to-body :visible.sync="kaoqinTag">
      ...
      <el-table :data="tableData" height="400" border style="width: 60%;margin: 0 auto;">
        ...
        <el-table-column align="center" label="考核人数">
          <template slot-scope="{ row }">
            <span v-show="mxsczt != 0">{{ row.kaohrs }}</span>
            <el-input v-show="mxsczt == 0" v-model="row.kaohrs"></el-input>
          </template>
        </el-table-column>

        <el-table-column align="center" label='“好”人数'>
          <template slot-scope="{ row }">
            <span v-show="mxsczt != 0">{{ row.haors }}</span>
            <el-input v-show="mxsczt == 0" v-model="row.haors"></el-input>
          </template>
        </el-table-column>

        <!-- <el-table-column align="center" label='截止日期' width="180">
          <template slot-scope="{ row }">
            <span v-show="!editid">{{ row.jiezrq }}</span>
            <el-date-picker
              v-show="editid"
              v-model="row.jiezrq"
        ...
        <el-table-column align="center" width="200" label="备注">
          <template slot-scope="{ row }">
            <span v-show="mxsczt != 0">{{ row.beiz }}</span>
            <el-input v-show="editid && mxsczt == 0" v-model="row.beiz" style="text-align: center;"></el-input>
          </template>
        </el-table-column>
      ...
      <div class="footer">
        <el-button type="primary" plain @click="savekq" v-if="editid && mxsczt == 0" :loading="btnLoading">保存</el-button>
    ...
    <HuilanDialog :title="csTitle" :fullscreen="true" width="80%" append-to-body :visible.sync="chushiDetail">
      ...
      <div class="csdate">{{ form1.tianbny1 }}</div>
      ...
      <div style="width: 100%;padding:10px">
        <table align="center" width="100%" class="table xunw_table_form" border="1">
          <tr>
            ...
            <td>{{ form1.xingm }}</td>
            <td>性别</td>
            <td v-if="form1.xingb == 1">男</td>
            <td v-if="form1.xingb == 2">女</td>
            <td>出生年月</td>
            <td>{{ form1.chusny }}</td>
          </tr>
          <tr>
            <td>处室及职务</td>
            <td colspan="5">{{ form1.suosbm }}</td>
          </tr>
          <tr>
            <td>考核指标</td>
          ...
          <tr>
            ...
            <td v-if="form1.zhengzpz == 0">好</td>
            <td v-if="form1.zhengzpz == 1">较好</td>
            <td v-if="form1.zhengzpz == 2">一般</td>
            <td v-if="form1.zhengzpz == 3">较差</td>
          </tr>
          <tr>
            <td>职业道德</td>
            <td colspan="4">忠于职守，服从命令，精通业务，勤勉敬业，遵守公务员职业道德，依法执行公务。</td>
            <td v-if="form1.zhiydd == 0">好</td>
            <td v-if="form1.zhiydd == 1">较好</td>
            <td v-if="form1.zhiydd == 2">一般</td>
            <td v-if="form1.zhiydd == 3">较差</td>
          </tr>
          <tr>
            <td>工作作风</td>
            <td colspan="4">热爱本职工作，敢于担当，积极主动，尽职尽责，求真务实，深入基层，服务企业。</td>
            <td v-if="form1.gongzzf == 0">好</td>
            <td v-if="form1.gongzzf == 1">较好</td>
            <td v-if="form1.gongzzf == 2">一般</td>
            <td v-if="form1.gongzzf == 3">较差</td>
          </tr>
          <tr>
            <td>廉洁自律</td>
            <td colspan="4">严格遵守《廉政准则》、中央八项规定、省委省政府20条意见，无不正之风或违纪违规现象。</td>
            <td v-if="form1.lianjzl == 0">好</td>
            <td v-if="form1.lianjzl == 1">较好</td>
            <td v-if="form1.lianjzl == 2">一般</td>
            <td v-if="form1.lianjzl == 3">较差</td>
          </tr>
          <tr>
            <td>个性指标</td>
            <td colspan="4">工作数量、质量、效率以及所产生的效益和业务能力等。</td>
            <td v-if="form1.gexzb == 0">好</td>
            <td v-if="form1.gexzb == 1">较好</td>
            <td v-if="form1.gexzb == 2">一般</td>
            <td v-if="form1.gexzb == 3">较差</td>
          </tr>
          <tr>
          ...
          <tr>
            <td >主管领导评鉴</td>
            <td  v-if="form1.zhugldpj == 0">好</td>
            <td  v-if="form1.zhugldpj == 1">较好</td>
            <td  v-if="form1.zhugldpj == 2">一般</td>
            <td  v-if="form1.zhugldpj == 3">较差</td>
            <td >（签名）</td>
            <td >{{form1.qianm}}</td>
            <td >日期：</td>
            <td >{{form1.pingjrq}}</td>
          ...
          <tr>
            <td >政治部审定</td>
            <td >{{ form1.zhengzbsd }}</td>
            <td >（签章）</td>
            <td ></td>
            <td >日期：</td>
            <td >{{ form1.zhengzbsdrq }}</td>
          </tr>
        </table>
    ...
    <!--各部门明细表汇总-->
    <HuilanDialog :title="mxhzTitle" :fullscreen="true" width="80%" append-to-body :visible.sync="mzhxDetail" @close="handleCloseDetail">
      <avue-crud v-if="mxhzshow" :option="optionMx" :table-loading="loadingMx" :data="dataMx" :page="pageMx" :permission="permissionList"
      :before-open="beforeOpenMx" v-model="formMx" ref="crudMx"
      @search-change="searchChangeMx" @search-reset="searchResetMx" @selection-change="selectionChangeMx"
      @current-change="currentChangeMx" @size-change="sizeChangeMx" @on-load="onLoadMx">
        <template v-slot:menu="scope">
          <el-button type="text" size="small" @click="mingxihzDetail(scope.row)">详情
          </el-button>
        </template>
      </avue-crud>
    </HuilanDialog>
    ...
    <!--余额依据表-->
    <HuilanDialog :title="yueTitle" :fullscreen="true" width="80%" append-to-body :visible.sync="yueDetail" @close="yueClose">
      <avue-crud :option="optionyue" :table-loading="loadingyue" :data="datayue" :page="pageyue" :permission="permissionList"
        ...
        @current-change="currentChangeyue" @size-change="sizeChangeyue" @on-load="onLoadyue" v-if="yueDetail">
        <template v-slot:menu="scope">
          <el-button type="text" size="small" @click="yueView(scope.row,scope.index)">查看
          </el-button>
          <el-button type="text" size="small" v-if="yuezt == 0" @click="yueUpdate(scope.row,scope.index)">编辑
          </el-button>
        </template>
      </avue-crud>
...
export default {
  ...
  methods: {
    ...
    //明细汇总
    handleHzmx(row){
      this.mzhxDetail = true
      this.mxhzshow = true
      this.mxhzId = row.id
      this.mxkhnd = row.khnd
      this.mxkaohjd = row.kaohjd
    },
    searchResetMx () {
      this.queryMx = {};
      this.onLoadMx(this.page);
    },
    searchChangeMx (params, done) {
      this.queryMx = params;
      this.pageMx.currentPage = 1;
      this.onLoadMx(this.pageMx, params);
      done();
    },
    selectionChangeMx (list) {
      this.selectionListMx = list;
    },
    selectionClearMx () {
      this.selectionListMx = [];
      this.$refs.crudMx.toggleSelection();
    },
    currentChangeMx (currentPage) {
      this.pageMx.currentPage = currentPage;
    },
    sizeChangeMx (pageSize) {
      this.pageMx.pageSize = pageSize;
    },
    refreshChangeMx () {
      this.onLoadMx(this.pageMx, this.queryMx);
    },
    ...
    onLoadMx (page) {
      ...
      const dataParam = {
        ...
        size: page.pageSize,
        khnd:this.mxkhnd,
        kaohjd:this.mxkaohjd,
        ...this.queryMx,
      };
      kaoheDetailSum(dataParam).then(res => {
        const data = res.data.data;
        this.pageMx.total = data.total;
        this.dataMx = data.records;
        this.loadingMx = false;
        this.selectionClearMx();
      });
    },
    mingxihzDetail(row){
    ...
    handleHuizong(row){
      const params = new URLSearchParams();
      params.append("id",row.id);
      download1(params).then(res =>{
        if(res.data.code == 200){
          var s1 = res.data.data
    ...
    comFormMagYue(row){
      this.yueDetail = true
      this.yueId = row.id
      this.yuezt = row.fabzt
    },
    yueClose(){
      this.yueDetail = false
      this.datayue = []
    },
    onLoadyue () {
