<template>
  <basic-container>
    ...
    > 
      ...
      @on-load="onLoadSb" :upload-preview="uploadPreviewSb" v-if="xinxiShow">
        <template v-slot:menu="scope">
  ...
  methods: {
    list(){
      ...
      getList(dataParam).then(res => {
        this.optionSb.column[0].dicData.push(...res.data.data.records)
    ...
    infoview(row,index){
      this.$refs.crudSb.rowView(row, index)
    },
    searchResetSb () {
      this.querySb = {};
