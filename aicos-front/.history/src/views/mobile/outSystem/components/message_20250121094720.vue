  ...
  <van-form @submit="onSubmit">
    <div v-for="(item,index) in list" :key="index">
      ...
      <van-field :label="item.label" :placeholder="item.label" v-if="item.type == 'upload'">
        <template #input>
          <van-uploader :disabled="disabled"  v-model="uploadImg" :multiple="multiple" :max-count="item.limit" :max-size="item.fileSize * 1024" :after-read="afterRead" :accept="item.accept"/>
  ...
  methods: {
    ...
    afterRead(file) {
      ...
      uploadImg(forms).then(res =>{
        this.list.forEach(item => {
          if(item.type == 'upload'){
            this.options[item.prop] = res.data.data.new
          }
        })
      })
    },
