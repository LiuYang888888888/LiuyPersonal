      ...
      >
        <van-swipe-cell v-for="item in list" :key="item" >
          ...
          <template #right>
            ...
            <van-button square type="danger" text="编辑"  v-if="(item.zhuangt == 1 || scope.row.zhuangt == 3) && permissionList.editBtn" @click="comFormMagUpdate(item)"/>
            <van-button square type="danger" text="提交"  v-if="(scope.row.zhuangt == 1 || scope.row.zhuangt == 3) && permissionList.tijiaoBtn" @click="comFormTijiao(item)"/>
            <van-button square type="danger" text="删除"  v-if="(scope.row.zhuangt == 1 || scope.row.zhuangt == 3) && permissionList.delBtn" @click="comFormMagDel(item)"/>
          </template>
        </van-swipe-cell>
      </van-list>
    </van-pull-refresh>
