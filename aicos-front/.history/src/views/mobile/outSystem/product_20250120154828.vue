      ...
      >
        <van-cell-group inset>
          <van-swipe-cell v-for="item in list" :key="item" >
            <van-cell :border="false" :title="item.chanpmc" :label="item.shenhly" :value="item.zt"/>
            <template #right>
              ...
              <van-button square type="danger" text="编辑"  v-if="permissionList.updateBtn" @click="comFormMagUpdate(item)" style="border-right: 1px solid #fff;"/>
...
import {remove, getList, submit} from "@/api/outSystemProduct/common";
import {
  apiData
    ...
    permissionList () {
      return {
        addBtn: this.vaildData(
          this.permission["sys-porduct-jyadd"],
          false
        ),
        viewBtn: this.vaildData(
          this.permission["sys-porduct-jyview"],
          false
        ),
        updateBtn: this.vaildData(
          this.permission[`sys-porduct-jyupdate`],
          false
        ),
        delBtn: this.vaildData(
          this.permission[`sys-porduct-jydelete`],
          false
        ),
        // 提交
        submit: this.vaildData(
          this.permission[`sys-porduct-jysubmit`],
          false
        ),
        // 审核
        examineBtn: this.vaildData(
          this.permission[`sys-porduct-jyexamineBtn`],
          false
        ),
      };
    },
