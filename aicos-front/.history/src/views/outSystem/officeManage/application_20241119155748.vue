<template>
  <basic-container>
    ...
    <HuilanDialog :title="appTitle" :fullscreen="false" append-to-body :before-close="appClose"
      ...
      <div style="width: 30%;margin:auto;text-align: center;" v-if="showSure">
...
export default {
            ...
            change: ({value}) =>{
              if(value){
                ...
                getListDataPro(params).then(res =>{ 
                  this.options.column[3].dicData = res.data.data;
                })
              }
            }
          },
          {
            ...
            change: ({value}) =>{
              if(value){
                ...
                getListDataJungong(dataParam).then(res =>{ 
                  this.form.keysl = res.data.data[0].kuckysl;
                  this.form.danj = res.data.data[0].danj;
                  this.form.danw = res.data.data[0].danw;
                })
              }
            }
          },
    ...
    permissionList () {
      return {
        toDoBtn: this.vaildData(
          this.permission[`outsys-application-toDo`],
          false
        ),
        doneBtn: this.vaildData(
          this.permission[`outsys-application-done`],
          false
        ),
        addBtn: this.vaildData(
          this.permission[`outsys-application-add`],
          false
        ),
        viewBtn: this.vaildData(
          this.permission[`outsys-application-view`],
          false
        ),
        editBtn: this.vaildData(
          this.permission[`outsys-application-edit`],
          false
        ),
        delBtn: this.vaildData(
          this.permission[`outsys-application-del`],
          false
        ),
        tijiaoBtn: this.vaildData(
          this.permission[`outsys-application-tijiao`],
          false
        ),
        shenheBtn: this.vaildData(
          this.permission[`outsys-application-shenhe`],
          false
        ),
      };
    },
  ...
  methods: {
    ...
    async meet(){
      ...
      this.options.column[2].dicData = data.data.data
    },
    tabChange () {
      this.query = {};
    ...
    //添加物品
    addWupin(){
      if(this.form.keysl > 0){
        if(this.form.keysl < this.form.shul){
          this.$message({
            type: "warning",
            message: "物品可用数量不足，请重新填写!"
          });
        }else{
          let obj = {}
          let a = BigNumber(this.form.shul);
          let b = BigNumber(this.form.danj);
          this.form.zongj = a.multipliedBy(b).toNumber()
          obj = {...this.form}
    ...
    async comFormMagView (row, index) {
      ...
      if (this.tabType !== 'DONE') {
        if(row.shiftj == "草稿"){
          ...
          this.appTag = true
          this.showform = true
          this.options.column[2].display = false
          this.options.column[3].display = false
          this.options.column[4].display = false
    ...
    saveList(){
      ...
      const dataParam = {
        ...
        godDetailApplicationList:this.wupinList
      };
      if(this.editId){
        dataParam.id = this.editId
      }
      if(this.wupinList.length == 0 && !dataParam.id){
        this.$message({
          type: "warning",
          message: "请先添加办公物品!"
        });
