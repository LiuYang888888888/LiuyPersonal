  ...
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template slot="menuLeft">
        ...
        <el-button size="small" type="primary" v-print="'#printTable'">打印</el-button>
      </template>
      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.shifck == 0">未出库</span>
            <span v-if="scope.row.shifck == 1">已出库</span>
          </template>
        </el-table-column>
      </el-table>
    </div>
