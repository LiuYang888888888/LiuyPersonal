        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="会议标题：" prop="huiybt">
              <el-input  placeholder="请输入会议标题" :readonly="true" v-model="form.huiybt"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item label="参会人员：" prop="tongzry">
              ...
              <div style="max-height:300px;overflow: auto;" v-if="lianxdhshow">
                <table align="center" width="90%" class="table xunw_table_form" border="1" style="margin-bottom: 40px;">
                  <tr>
                    <td>编号</td>
                    <td>联系电话</td>
                  </tr>
    ...
    <HuilanDialog :title="huizhiTitle" :fullscreen="false" append-to-body :before-close="huizhiClose"
      ...
      <div style="max-height:400px;overflow: auto;">
        <table align="center" width="90%" class="table xunw_table_form" border="1" style="margin-bottom: 40px;">
          ...
          <tr v-for="(item,index) in huizhiData" :key="index">
            <td>{{index+1}}</td>
            <td>{{item.shoujh}}</td>
        ...
        <el-tag v-if="scope.row.shiftj == '审核通过'" effect="dark" type="success">审核通过</el-tag>
            <td v-if="item.duanxhzzt == 0">
              <el-tag effect="dark" type="info">未发送</el-tag>
            </td>
            <td v-if="item.duanxhzzt == 1">
              发送成功
              <el-tag v-if="scope.row.shiftj == '审核通过'" effect="dark" type="success">发送成功</el-tag>
            </td>
            <td v-if="item.duanxhzzt == 2">
              <el-tag effect="dark" type="danger">发送失败</el-tag>
            </td>
          </tr>
        </table>
      </div>
  ...
  methods: {
    ...
    comFormMagtz(row){
      ...
      const dataParam = {
        ...
        searchParam: {
          huiytzid:row.huiyryid
        },
      };
      viewListData(dataParam).then((res) => {
        const dataRes = res.data.data;
        this.huizhiData = dataRes.fieldList
      });
    },
    initData (/*type*/) { },
    ...
    async comFormMagView (row) {
      ...
      this.form.huiybt = row.huiybt
      this.form.tongznr = row.tongznr
      this.form.tableList = row.meetingPerson
