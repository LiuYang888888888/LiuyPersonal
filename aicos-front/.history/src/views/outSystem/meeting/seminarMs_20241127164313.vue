  ...
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.tzBtn && scope.row.zhuangt == 2" @click="comFormMagtz(scope.row)">审核
        </el-button>
    ...
    <!--审核弹窗-->
    <HuilanDialog :title="dataShenheTitle" :fullscreen="false" :modal-append-to-body="false" :before-close="dataShenheClose"
      :visible.sync="dataShenheTag">
        <div class="dataMagFormFooter">
          ...
          <div class="dataMagFormBtns">
            <el-button size="small" icon="el-icon-check" type="success" @click.stop="dataMagVerifyYes" :loading="btnshLoading">审核通过
            </el-button>
            <el-button size="small" icon="el-icon-close" type="warning"
              @click.stop="dataMagVerifyNo" :loading="btnshLoading">审核不通过
            </el-button>
            <el-button size="small" icon="el-icon-circle-close" @click="dataShenheClose">取 消</el-button>
          </div>
        </div>
    </HuilanDialog>
  </basic-container>
...
export default {
    ...
    permissionList () {
      return {
        viewBtn: this.vaildData(
          this.permission[`outsys-seminarMs-view`],
          false
        ),
        tzBtn: this.vaildData(
          this.permission[`outsys-seminarMs-tongzhi`],
          false
        ),
      };
    },
  ...
  methods: {
    ...
    onLoad (page /*, params = {}*/) {
      ...
      getListDataAll(dataParam).then((res) => {
    ...
    //审核关闭
    dataShenheClose(){
      this.dataShenheTag = false
      this.btnshLoading = false
      this.formVerifyText = ''
    },
    //审核弹窗
    comFormMagtz(row){
      this.dataShenheTag = true;
      this.dataFormId = row.id;
    },
    //审核通过
    dataMagVerifyYes(){
      this.btnshLoading = true;
      const params = new URLSearchParams();
