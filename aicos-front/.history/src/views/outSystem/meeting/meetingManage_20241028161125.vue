    ...
    <el-row :gutter="24">
      <el-col :span="8" v-for="(item, index) in listData" :key="index" style="margin-top: 20px;">
        <el-card :body-style="{ padding: '10px',background:'#f9f9f9'}" shadow="always">
          <div class="cardList">
            <div class="card-img">
              <img :src="'/aicos' + item.huiystp" v-if="item.huiystp"/>
              <img :src="cheimg" v-else/>
            </div>

            <div class="cardInfo">
              <div class="card-text carditem">会议室名称：{{ item.huiysmc }}</div>
              <div class="card-text carditem">会议室设备：{{ item.huiyssb }}</div>
              <div class="card-text carditem">会议室人数：{{ item.huiysrs }}人</div>
              <div class="card-text carditem">会议室位置：{{ item.huiyswz }}</div>
            </div>
          </div>
