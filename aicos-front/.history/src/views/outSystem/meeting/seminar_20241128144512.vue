  ...
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="permissionList.tzBtn && scope.row.zhuangt == 4 && scope.row.xftzzt == 2" @click="comFormMagtz(scope.row)">通知
        </el-button>
    ...
    <!--新增弹窗-->
    <HuilanDialog :title="dataMagFormTitle" :fullscreen="false" :modal-append-to-body="false" :before-close="dataMagFormClose"
      ...
      <el-form :rules="rules" ref="form" :model="form" label-width="120px" class="addform">
        <el-row>
          ...
          <el-col :span="24" v-if="!disableFlag">
    ...
    <!--通知弹窗-->
    <HuilanDialog :title="tongzhiTitle" :fullscreen="false" :modal-append-to-body="false" :before-close="tongzhiClose"
      :visible.sync="tongzhiTag">
      <el-form :rules="tzrules" ref="tzform" :model="tzform" label-width="120px" class="addform">
        <el-row>
          <el-col :span="24">
            <el-form-item label="会议标题：" prop="huiybt">
              <el-input  placeholder="请输入会议标题" :disabled="tzdisableFlag" v-model="tzform.huiybt"/>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="24">
            <el-form-item label="参会人员：" prop="tongzry">
              <el-upload :auto-upload="false" :show-file-list="false"
                     :on-change="choosePeople" style="display: inline-block;">
                <el-button size="small" icon="el-icon-upload2" type="primary" ref="comp_chose">导入</el-button>
              </el-upload>
              <el-button size="small" icon="el-icon-download" type="primary" @click="downloadExcel" style="margin-left:20px">
                下载模版
              </el-button>
            </el-form-item>
          ...
          <el-col :span="24">
            <el-form-item size="large" style="text-align: center;">
              <el-button type="primary" @click="tzonSubmit" :loading="tzbtnLoading">下发</el-button>
              <el-button @click="tongzhiClose">取消</el-button>
            </el-form-item> 
          </el-col>
        </el-row>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    downloadExcel() {
      ...
      a.href = `${baseUrl}template/会议通知人员手机号列表.xls`;
      a.download = "会议通知人员手机号列表.xls";
    ...
    //通知
    comFormMagtz(row){
      this.tongzhiTag = true
      this.tzid = row.id
      this.tzform.huiybt = row.shenqhysmc
    },
    //通知关闭
    tongzhiClose(){
      this.tongzhiTag = false
      this.tzform.huiybt = ''
      this.tzform.tongznr = ''
      this.tableList = []
    },
      ...
      if (this.beforeAvatarUpload(file)) {
        ...
        importExcel(formdata).then((res) => {
          if (res.status === 200) {
            ...
            this.tableList = res.data.data.meetingPersonList
            this.huiytzid = res.data.data.huiytzid
            this.lianxdhshow = true
          } else {
            this.$message({
              message: res.msg,
              type: 'error'
    ...
    //通知保存
    tzonSubmit(){
      this.$refs.tzform.validate((valid) => {
        if (valid) {
          this.tzbtnLoading = true
          this.tzform.huiyryid = this.huiytzid
          this.tzform.hysqid = this.tzid
          tzsaveOrUpdateListData(this.tzform).then(() => {
            this.tongzhiClose()
            this.onLoad(this.page);
            this.$message({
              type: "success",
            ...
            this.tzbtnLoading = false
            this.$router.push({
              path: "/outSystem/meeting/notice",
            });
          }, error => {
            window.console.log(error);
          });
