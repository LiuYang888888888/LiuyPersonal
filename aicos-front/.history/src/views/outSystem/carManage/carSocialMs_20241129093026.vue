  ...
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template v-slot:menu="scope">
        ...
        <el-button type="text" size="small" v-if="scope.row.shiftj == '审核通过' && permissionList.paicheBtn && scope.row.paichezt == '待派车'"
          @click="comFormMagPaiche(scope.row)">派车
        </el-button>
      </template>
    </avue-crud>
  </basic-container>
...
import {
  getListData,
  submitListData
} from "@/api/outSystem/carSocialMs";
import {
  getInfo,
    ...
    permissionList () {
      return {
        viewBtn: this.vaildData(
          this.permission[`outsys-carSocialMs-view`],
          false
        ),
        paicheBtn: this.vaildData(
          this.permission[`outsys-carSocialMs-paiche`],
          false
        ),
      };
    },
  ...
  methods: {
    ...
    beforeOpen (done, type) {
      if (["add", "edit", "view"].includes(type)) {
        if (type == 'edit') {
          this.listForm.chengbr = this.userInfo.account
        }
        this.initData(type);
      }
      done();
