  ...
  <div class="sysIndex">
    ...
    <div class="sysIndexBottom">
      <div class="sysIndexBottomCon">
        <lineshow :lineTitle="lineTitle" :areaData="areaData" :areaNum="areaNum">
          <template v-slot:header>
            <div class="soltBody">
              <div class="soltBodyContent" v-for="(item,index) in lineInfo" :key="index">
                <div class="soltBodyChart">{{ item.name }}</div>
                <div class="soltBodyFooter">{{ item.value }}</div>
              </div>
            </div>
          </template>
        </lineshow>
...
.soltBoxContent:first-child .soltBoxFooter{
  font-size: 18px;
  color: #af2c30;
}
</style>