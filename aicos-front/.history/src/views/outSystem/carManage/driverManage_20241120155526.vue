    ...
    <el-row :gutter="24">
      <el-col :span="8" v-for="(item, index) in listData" :key="index" style="margin-top: 20px;">
        <el-card :body-style="{ padding: '10px',background:'#f9f9f9'}" shadow="always">
          <div class="cardList">
            <div class="card-img">
              <img :src="'/aicos' +item.jiasyzp" v-if="item.jiasyzp"/>
              <img :src="cheimg" v-else/>
            </div>

            <div class="cardInfo">
              <div class="card-text carditem">姓名：{{ item.xingm }}</div>
              <div class="card-text carditem">驾龄：{{ item.jial }}年</div>
              <div class="card-text carditem">联系电话：{{ item.lianxdh }}</div>
...
export default {
  ...
  methods: {
    async carListMethod(){
      ...
      this.listOption.column[4].dicData = data.data.data.fieldList
    },
