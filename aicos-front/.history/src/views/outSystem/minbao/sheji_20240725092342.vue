    ...
    <div class="mod-menu-home app-container">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" v-if="dialog">
        <el-form-item label="问卷名称：" prop="pname">
          <el-input v-model="ruleForm.pname" style="width:60%" placeholder="请输入问卷名称" maxlength="100" :disabled="look"></el-input>
        </el-form-item>
        <el-form-item label="问卷说明：" prop="descrip">
          <el-input v-model="ruleForm.descrip" style="width:60%" placeholder="请填写问卷说明（200字以内）" type="textarea"
            :rows="4" maxlength="200" :disabled="look"></el-input>
        </el-form-item>
          ...
          <ul class="content-ul">
            <li v-for="(item,index) in questionList" :key="item.id">
              <dl v-if="item.qtype == 'radio'">
                ...
                <dt>{{ item.qcontent }}  <span style="color: #409eff;">（单选）</span></dt>
                <div class="newradio">
      ...
      item.optionContent }}</el-radio>
              ...
              <dl v-if="item.qtype == 'checkbox'">
                <dt :class="item.ismust == 1 ? 'require' : ''" style="float: left;">{{ index + 1 }}. </dt>
                <dt>{{ item.qcontent }}  <span style="color: #409eff;">（多选）</span></dt>
                <div>
                  <el-checkbox-group v-model="checkList[index]" :key="index">
                    <el-checkbox :label="item.optionContent" v-for="(item, index) in item.questionOptionVOList" :key="index"></el-checkbox>
                  </el-checkbox-group>
                </div>
              </dl>
      ...
      <!-- 新增内容弹框 -->
      <el-dialog title="新增内容" :visible.sync="dialogVisible" width="40%" :before-close="cancelDialog" :append-to-body="true" v-if="dialogVisible">
        <div>
...
<script>
import {questionList,getQuestion,delQuestion,delOption} from "@/api/project_shenpi_config/shenpiconfig";
export default {
  data () {
    return {
      // 查询参数
    ...
    dataFormSubmit () {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          const request = this.spid ? questionList(JSON.stringify(this.ruleForm)) : questionList(JSON.stringify(this.ruleForm))
          request.then(() => {
            this.$message({
              message: this.$t('操作成功！'),
              type: 'success',
