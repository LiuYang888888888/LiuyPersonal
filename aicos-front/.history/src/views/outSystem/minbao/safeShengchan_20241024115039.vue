  ...
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template #expiry="scope">
        <el-tag v-if="scope.row.expiry == 0" effect="dark" type="danger" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">即将到期</el-tag>
        <el-tag v-if="scope.row.expiry == 1" effect="dark" type="danger" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">即将到期</el-tag>
        <el-tag v-if="scope.row.expiry == 2" effect="dark" type="danger" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">已过期</el-tag>
      </template>
