<template>
  <basic-container>
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button @click="handleStart(scope.row)" type="text" size="small" v-if="scope.row.wjstatus =='停用'">启用</el-button>
            <el-button @click="handleStart(scope.row)" type="text" size="small" v-else>停用</el-button>
            <el-button @click="handleFb(scope.row)" type="text" size="small" v-if="scope.row.fbstatus =='未发布'">发布</el-button>
    ...
    <el-dialog title="问卷统计" :visible.sync="dialogTableVisible" :modal="true"
    :before-close="handleDialogClose" width="60%" :lock-scroll="true" :append-to-body="true">
        ...
        <el-table :data="item.voList" border :header-cell-style="{background:'#eef1f6',color:'#606266'}">
          ...
          <el-table-column property="optionTitle" label="选项">
            <template scope="scope">
              <span v-if="scope.row.optionTitle === '问答题结果'" style="color: #409eff; cursor: pointer;" @click="handleResult(item.questionId)">{{ scope.row.optionTitle  }}</span>
    ...
    <Dialogsp ref="dialogFormsp" :dialog.sync="dialogFormVisiblesp" :spid="rowspid" @closesp="closesp" @closespcg="closespcg" :look="lookView"/>

  </basic-container>
</template>

<script>
  import Dialog from "@/views/project_shenpi_config/mylook.vue";
  import Dialogsp from "@/views/policy_dynamics/sheji.vue";
  import {shejiList,editStatus,copyQuestion,getQuestionStatistics,exportQuestion} from "@/api/project_shenpi_config/shenpiconfig";
