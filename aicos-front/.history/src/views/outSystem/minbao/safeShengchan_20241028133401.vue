<template>
  <basic-container>
    <avue-crud :option="listOption" :table-loading="loading" :data="listData" ref="listRef" v-model="listForm"
      ...
      <template #expiry="scope">
        <el-tag v-if="scope.row.expiry == 0" effect="dark" type="success" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">正常</el-tag>
        <el-tag v-if="scope.row.expiry == 1" effect="dark" type="warning" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">即将到期</el-tag>
        <el-tag v-if="scope.row.expiry == 2" effect="dark" color="#af2c30" style="cursor: pointer;display: inline-block" :title="scope.row.expiryString">已过期</el-tag>
      </template>
